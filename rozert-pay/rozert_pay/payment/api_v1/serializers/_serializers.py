    ...
    def create(self, validated_data: dict[str, ty.Any]) -> PaymentTransaction:
        ...
        BalanceUpdateService.update_balance(
            BalanceUpdateDTO(
                ...
                event_type=BalanceTransactionType.SETTLEMENT_REQUEST,
                amount=Money(validated_data["amount"], validated_data["currency"]),
                payment_transaction=trx,
                initiator=InitiatorType.USER,
                description=f"User withdrawal request via API for wallet {wallet.uuid}",
            )
        )
...
class BitsoSpeiCardBankSerializer(serializers.ModelSerializer):
    class Meta:
...
class CardBinDataSerializer(serializers.ModelSerializer):
    bank = BankSerializer()
    bitso_banks = BitsoSpeiCardBankSerializer(many=True, read_only=True)
