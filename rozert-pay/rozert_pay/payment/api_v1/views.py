...
from rest_framework import mixins, viewsets  # type: ignore
...
from rozert_pay.common.authorization import (
    BetmasterSecretKeyAuthentication,
    HMACAuthentication,
)
...
class GenericPaymentSystemApiV1Mixin:
    ...
    def _generic_withdraw(
        ...
        # If True - expect our customer external account UUID instead of third-party account
        # TODO: pass system type and use controller value instead
        use_our_uuid_for_customer_external_account_to_withdraw: bool = False,
    ) -> Response:
        ...
        if use_our_uuid_for_customer_external_account_to_withdraw:
            validated_data[
                "withdraw_to_account"
            ] = payment_types.CustomerExternalAccountUuid(
                validated_data["withdraw_to_account"]
            )

        trx = serializer.create(validated_data)
    ...
    def _generic_create_instruction(
        ...
        *,
        ...
        if isinstance(result, errors.Error):
            logger.warning(f"Error creating instruction: {result}")
            raise ValidationError("Unable to create instruction")
...
class CardBinDataViewSet(viewsets.ReadOnlyModelViewSet):
    authentication_classes = [BetmasterSecretKeyAuthentication]
    permission_classes = [IsAuthenticated]
    serializer_class = CardBinDataSerializer
    pagination_class = StandardResultsSetPagination

    def get_queryset(self) -> QuerySet[PaymentCardBank]:
        return PaymentCardBank.objects.select_related("bank").prefetch_related(
            "bitso_banks"
        )
