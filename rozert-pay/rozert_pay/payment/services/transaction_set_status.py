...
from rozert_pay.payment.permissions import PaymentPermissions
...
class BaseTransactionSetter(Generic[T]):
    ...
    @track_duration("BaseTransactionSetter.get_form")
    def get_form(self, data: dict[str, Any]) -> Error | T:
        if not PaymentPermissions.CAN_SET_TRANSACTION_STATUS.allowed_for(
            self.initiator
        ):
    ...
    @errors.wrap_errors
    @track_duration("BaseTransactionSetter.save_form")
