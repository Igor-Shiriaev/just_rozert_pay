...
from bm.django_utils.db_functions import JSONFieldSetValueFunc
...
@track_duration("db_services.get_transaction")
def get_transaction(
    *,
    for_update: bool,
...
@track_duration("db_services.create_transaction")
def create_transaction(
    ...
    # If set, customer_external_account will be created and bound to transaction.
    customer_external_account_number: str | None | types.CustomerExternalAccountUuid,
    user_data: dict[str, Any] | None,
    card_data: dict[str, Any] | None,
    # If set, customer_external_account will be created and bound to transaction.
    customer_id: types.ExternalCustomerId | None = None,
    extra: dict[str, Any] | None = None,
    # ====== attributes below are only for transactions created by callback =========
    # Should be set only if we already know id_in_payment_system. For example like in bitso_spei
    # when we create transaction on callback.
    id_in_payment_system: str | None = None,
    # If set, customer instruction will be bound to transaction. This is needed only for transactions
    # created on callbacks, like SPEI.
    ...
    if customer_external_account_number:
        ...
        if controller.use_our_uuid_for_external_account_number:
            customer_account = CustomerExternalPaymentSystemAccount.objects.get(
                uuid=customer_external_account_number
            )
        else:
            # It's not our UUID, so find or create new customer_external_account_number
            customer_account = find_customer_external_account_by_number(
                account_number=str(customer_external_account_number),
                system_type=system_type,
            )
...
@track_duration("db_services.save_extra_field")
def save_extra_field(trx: PaymentTransaction, field: str, value: Any) -> None:
    PaymentTransaction.objects.filter(id=trx.id).update(
        extra=JSONFieldSetValueFunc(
            "extra",
            key=field,
            value=value,
            create_missing=True,
    ...
    trx.extra[field] = value


@track_duration("db_services.create_card")
def create_card(
    *,
...
@track_duration("db_services.get_card_by_token")
...
@track_duration("db_services.get_wallet")
def get_wallet(
...
@track_duration("db_services.lock_currency_wallet")
...
@track_duration("db_services.create_customer_external_payment_system_account")
...
@track_duration("db_services.create_customer_deposit_instruction")
...
@track_duration("db_services.filter_customer_external_payment_system_accounts")
...
@track_duration("db_services.find_customer_external_account_by_number")
...
@track_duration("db_services.find_deposit_instruction_by_account")
