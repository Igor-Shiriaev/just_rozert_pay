...
@track_duration("transaction_status_validation.validate_remote_transaction_status")
...
@track_duration("transaction_status_validation._validate_status")
def _validate_status(
    transaction: PaymentTransaction, remote_status: entities.RemoteTransactionStatus
) -> errors.Error | None:
    if remote_status.operation_status in [
        const.TransactionStatus.CHARGED_BACK,
        const.TransactionStatus.REFUNDED,
    ]:
        ...
        if transaction.type != const.TransactionType.DEPOSIT:
            return errors.Error(
                f"received chargeback for non-deposit transaction {transaction.type}"
            )
        # Chargeback on successful deposit
        return None

    if remote_status.operation_status == const.TransactionStatus.CHARGED_BACK_REVERSAL:
        if transaction.status != const.TransactionStatus.CHARGED_BACK:
            return errors.Error(
                f"received chargeback reversal for non-chargeback transaction {transaction.status}"
            )
        if transaction.type != const.TransactionType.DEPOSIT:
            return errors.Error(
                f"received chargeback reversal for non-deposit transaction {transaction.type}"
            )
        # Chargeback reversal on chargeback
        return None
...
@track_duration("transaction_status_validation.disable_final_status_validation")
...
@track_duration("transaction_status_validation.is_final_status_validation_enabled")
...
class TransactionAmountValidation:
    @classmethod
    @track_duration(
        "TransactionAmountValidation.bypass_amount_validation_for_transaction"
    )
...
@track_duration("transaction_status_validation._validate_amounts")
...
@track_duration("transaction_status_validation.bypass_validation")
