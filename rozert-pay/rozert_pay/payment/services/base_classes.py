...
from rozert_pay.payment.models import PaymentSystem, PaymentTransaction, Wallet
from rozert_pay.payment.services import db_services, errors, incoming_callbacks
...
class BasePaymentClient(ty.Generic[T_Credentials]):
    ...
    def get_transaction_status(self) -> RemoteTransactionStatus | errors.Error:
        if not settings.IS_UNITTESTS:
            safe_assert(
                ...
                "This method should not be called in transaction",
            )

        try:
            return self._get_transaction_status()
        except errors.NoSentryException as e:
            logger.warning("Error getting transaction status", exc_info=True)
    ...
    def setup_webhooks(
        ...
        *,
        ...
        assert url or system or wallet

        if not url:
            if system:
                url = incoming_callbacks.get_rozert_callback_url(system=system)
            else:
                if wallet is None:
                    raise AssertionError("Wallet is required when system/url missing")
                wallet = ty.cast(Wallet, wallet)
                url = incoming_callbacks.get_rozert_callback_url(system=wallet.system)
        ...
        if remove_existing:
            cls.remove_webhooks(urls_=remove_pattern, creds=creds, log_writer=logger)
        ...
        for webhook in cls.get_webhooks(creds):
            logger.write(f"Webhook: {webhook.raw_data}")
