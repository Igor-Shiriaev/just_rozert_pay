...
from rozert_pay.payment.services import transaction_processing
...
@track_duration("cardpay_callback_parsing._calc_cardpay_signature")
...
class CardpayCallbackServices:
    @classmethod
    @track_duration("CardpayCallbackServices.parse_cardpay_callback")
    def parse_cardpay_callback(cls, cb: IncomingCallback) -> RemoteTransactionStatus:
        body = cb.body
    ...
    @classmethod
    @track_duration("CardpayCallbackServices.validate_callback_signature")
    def validate_callback_signature(
        cls, cb: IncomingCallback, payment_system: const.PaymentSystemType
    ) -> bool:
