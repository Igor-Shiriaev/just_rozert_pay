...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def deposit(self) -> entities.PaymentClientDepositResponse:
        ...
        trx_extra: dict[str, ty.Any] = self.trx.extra
        assert WorldpayTransactionExtraFields.SESSION_ID in trx_extra
        assert WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE in trx_extra
        ...
        xml_payload: str = generate_worldpay_xml(
            self._build_deposit_payload(
                ...
                request_3ds_challenge=bool(
                    trx_extra[WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE]
                ),
            )
        )
        ...
        if "challengeRequired" in order_status:
            if not bool(
                trx_extra.get(WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE)
            ):
                logger.error(
        ...
        if bool(trx_extra.get(WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE)):
            logger.error(
