...
from bm.django_utils.metrics import safe_assert
...
from rozert_pay.common import const, metrics
...
class PaymentSystemController(Generic[T_Client, T_SandboxClient]):
    ...
    def __init__(
        ...
        *,
        ...
        # For withdrawals, use our CustomerExternalAccount.uuid for withdraw_to_account instead of provider account
        use_our_uuid_for_external_account_number: bool = False,
    ) -> None:
        ...
        self.use_our_uuid_for_external_account_number = (
            use_our_uuid_for_external_account_number
        )
    ...
    def run_deposit(self, trx_id: types.TransactionId) -> None:
        try:
            ...
            metrics.DEPOSIT_COUNT.labels(
                status=trx.status,
                system=trx.system.type,
            ).inc()
    ...
    def sync_remote_status_with_transaction(
        ...
        *,
        ...
        # Handle chargeback case
        if remote_status.operation_status == TransactionStatus.CHARGED_BACK:
            transaction_processing.handle_chargeback(
                trx=trx,
            )
        ...
        # Handle chargeback reversal case
        if remote_status.operation_status == TransactionStatus.CHARGED_BACK_REVERSAL:
            transaction_processing.handle_chargeback_reversal(
                trx=trx,
            )
            transaction.on_commit(
                lambda: self.create_callback(
        ...
        # Handle refund case
        if remote_status.operation_status == TransactionStatus.REFUNDED:
            assert (
                remote_status.refund_amount
            ), "Refund amount expected for transaction!"
            transaction_processing.handle_refund(
                trx=trx,
                refund_amount=remote_status.refund_amount,
            )
            transaction.on_commit(
                lambda: self.create_callback(
                    trx_id=trx_id,
            ...
            if allow_transition_from_final_statuses:
                ...
                if transaction_status_validation.is_final_status_validation_enabled():
                    safe_assert(
                        ...
                        f"Transaction status is not initial: {trx.status} != {remote_status.operation_status}",
                    )
                return
        ...
        if trx.status == TransactionStatus.SUCCESS:
            if trx.type == TransactionType.DEPOSIT:
                # A successful deposit increases the total (operational) funds
                # and the portion of funds awaiting settlement from the provider (pending)
                balance_tx_record = BalanceUpdateService.update_balance(
                    BalanceUpdateDTO(
                        currency_wallet=trx.wallet,
                        event_type=BalanceEventType.OPERATION_CONFIRMED,
                ...
                event_logs.create_transaction_log(
                    ...
                    extra={
                        ...
                        "pending_after": str(balance_tx_record.pending_after),
                    },
                )
                return
            ...
            # Successfull payout - decrease hold balance
                ...
                # This is the confirmation of a prior settlement request.
                # successful withdrawal means funds that were previously frozen
                # have now left system
                # TSETTLEMENT_CONFIRMED
                # event correctly reduces both the total (operational) balance
                # and the frozen balance. This is why it's different from a deposit
                balance_tx_record = BalanceUpdateService.update_balance(
                    BalanceUpdateDTO(
                        currency_wallet=trx.wallet,
                        event_type=BalanceEventType.SETTLEMENT_CONFIRMED,
                        amount=Money(trx.amount.copy_abs(), trx.currency),
                        payment_transaction=trx,
                        initiator=InitiatorType.SYSTEM,
                    )
                ...
                if trx.wallet.frozen_balance < 0:
                    logger.error(
                        "Frozen balance is negative after successful withdrawal",
                        extra={
                            "trx_id": trx_id,
                            "balance_transaction_id": str(balance_tx_record.id),
            ...
            if trx.type == TransactionType.WITHDRAWAL:
                # A failed withdrawal means that funds previously frozen for payout
                # must be returned to the available balance SETTLEMENT_CANCEL
                # event handles this by reducing the frozen balance
                balance_tx_record = BalanceUpdateService.update_balance(
                    BalanceUpdateDTO(
                        currency_wallet=trx.wallet,
                        event_type=BalanceEventType.SETTLEMENT_CANCEL,
                        amount=Money(trx.amount.copy_abs(), trx.currency),
                        payment_transaction=trx,
                        initiator=InitiatorType.SYSTEM,
                    )
                ...
                if trx.wallet.frozen_balance < 0:
                    logger.error(
                        "Frozen balance is negative after failed withdrawal",
                        extra={
                            "trx_id": trx_id,
                            "balance_transaction_id": str(balance_tx_record.id),
                            "frozen_after": str(balance_tx_record.frozen_after),
                ...
                event_logs.create_transaction_log(
                    trx_id=trx_id,
                    event_type=EventType.INFO,
                    description="Withdrawal failed, funds un-held.",
                    extra={
                        "balance_transaction_id": str(balance_tx_record.id),
                        "operational_after": str(balance_tx_record.operational_after),
                        "frozen_after": str(balance_tx_record.frozen_after),
    ...
    def run_withdraw(self, transaction_id: types.TransactionId) -> None:
        try:
            ...
            # Enqueue task to cleanup long running transactions
            # TODO: add withdrawal ttl functional
            # ttl_sec = trx.system.deposit_allowed_ttl_seconds
            # tasks.task_fail_by_timeout.apply_async(
            #     args=(trx.id, ttl_sec),
            #     countdown=ttl_sec + 5,
