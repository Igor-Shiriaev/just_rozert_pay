...
def replace_non_ascii_chars(s: str) -> str:
    result = unidecode(s).replace("'", "")

    # remove all punctuation except for dash
    result = re.sub(r"[^\w\s-]", "", result)

    return result.strip()


class AppexClient(base_classes.BasePaymentClient[AppexCreds]):
    payment_system_name = const.PaymentSystemType.APPEX
    credentials_cls = AppexCreds
            ...
            if "PaReq" in parsed_data:
                redirect_form_data = entities.TransactionExtraFormData(
                    ...
                    fields={
                        ...
                        "MD": parsed_data["MD"],
                        "TermUrl": deposit_services.get_return_url(
    ...
    def _check_pay_status(
        self, trx: PaymentTransaction, creds: AppexCreds
    ) -> dict[str, ty.Any]:
        payload = {
            "opertype": "check",
            "number": trx.uuid.hex,
    ...
    def _sign_payload(
        ...
        *,
        ...
        str_data = ":".join(data)
        result = hashlib.md5(str_data.encode("utf8")).hexdigest().upper()
        logger.info(
            "Appex payload to sign",
            extra={
                "str_data": str_data,
                "result": result,
            },
...
class AppexSandboxClient(base_classes.BaseSandboxClientMixin[AppexCreds], AppexClient):
    pass
