...
from rozert_pay.limits.models import CustomerLimit, LimitCategory, MerchantLimit
...
@track_duration("limits.invalidate_limits_cache")
...
def get_active_limits() -> (
    ...
    active_limits = memory_cache_get_set(
...
@track_duration("limits._should_check_customer_limit")
...
@track_duration("limits._should_check_merchant_limit")
...
@track_duration("limits._process_transaction_limits")
...
@track_duration("limits.check_limits_and_maybe_decline_transaction")
...
@track_duration("limits._check_customer_limit")
def _check_customer_limit(
    ...
    if limit.min_operation_amount and trx.amount < limit.min_operation_amount:
        ...
        ] = f"Transaction amount {trx.amount} is less than limit {limit.min_operation_amount.quantize(Decimal('.01'))}"
    ...
    if limit.max_operation_amount and trx.amount > limit.max_operation_amount:
        ...
        ] = f"Transaction amount {trx.amount} is greater than limit {limit.max_operation_amount.quantize(Decimal('.01'))}"
        ...
        if is_limit_triggered:
            logger.info(
                ...
                extra={
                    ...
                    "triggers_data": json.dumps(triggers_data),
                },
            )
        else:
            logger.info(
                "Limit has not been triggered",
                extra={
                    "transaction_id": trx.id,
                    "limit_id": limit.id,
                    "limit_category": limit.category,
    ...
    if limit.total_successful_amount:
        ...
        if (
            ...
            triggers_data["total_successful_amount"] = (
                ...
                f"transaction amount {trx.amount} is greater than limit {limit.total_successful_amount.quantize(Decimal('.01'))}"
            )
...
@track_duration("limits._check_merchant_limit")
def _check_merchant_limit(
    ...
    if limit.limit_type == limit_models.LimitType.MIN_AMOUNT_SINGLE_OPERATION and (
        ...
        if trx.amount < limit.min_amount:
            is_limit_triggered = True
            triggers_data[limit_models.LimitType.MIN_AMOUNT_SINGLE_OPERATION.label] = (
                f"Transaction amount {trx.amount} is less than limit "
                f"{limit.min_amount.quantize(Decimal('0.01'))}"
            )
    ...
    if limit.limit_type == limit_models.LimitType.MAX_AMOUNT_SINGLE_OPERATION and (
        ...
        if trx.amount > limit.max_amount:
            is_limit_triggered = True
            triggers_data[limit_models.LimitType.MAX_AMOUNT_SINGLE_OPERATION.label] = (
                f"Transaction amount {trx.amount} is greater than limit "
                f"{limit.max_amount.quantize(Decimal('0.01'))}"
            )
            triggers_data["scope"] = limit.scope
    ...
    if (
        ...
        if successful_deposits_amount + trx.amount > limit.total_amount:
            ...
            triggers_data[limit_models.LimitType.TOTAL_AMOUNT_DEPOSITS_PERIOD.label] = (
                ...
                f"amount {trx.amount} is greater than limit "
                f"{limit.total_amount.quantize(Decimal('0.01'))} for period {limit.period}"
            )
    ...
    if (
        ...
        if successful_withdrawals_amount + trx.amount > limit.total_amount:
            ...
            triggers_data[
                ...
                trx.amount.quantize(Decimal(".01")),
                limit.total_amount.quantize(Decimal("0.01")),
                limit.period,
            )
...
@track_duration("limits._get_start_date_of_limit")
...
@track_duration("limits._resolve_customer_and_merchant_limit_conflicts")
...
@track_duration("limits._notify_about_alerts")
