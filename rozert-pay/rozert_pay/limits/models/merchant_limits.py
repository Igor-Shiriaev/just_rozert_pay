...
from rozert_pay.limits.models import LimitCategory
...
class MerchantLimit(BaseDjangoModel):
    ...
    min_amount = fields.MoneyField(  # type: ignore[misc]
        null=True,
        blank=True,
        validators=[MinValueValidator(0)],
        help_text="Minimum amount",
    )
    max_amount = fields.MoneyField(  # type: ignore[misc]
        null=True,
        blank=True,
        validators=[MinValueValidator(0)],
        help_text="Maximum amount",
    )
    total_amount = fields.MoneyField(  # type: ignore[misc]
        null=True,
        blank=True,
        validators=[MinValueValidator(0)],
    ...
    def clean(self) -> None:
        ...
        if self.category == LimitCategory.GLOBAL_RISK and (
            ...
            if self.scope != MerchantLimitScope.WALLET:
                errors.setdefault("scope", []).append(
                    "Scope must be 'wallet' for global risk limit"
                )
            if not self.wallet:
                errors.setdefault("wallet", []).append(
                    "Wallet must be set for global risk limit"
                )
...
class GlobalRiskMerchantLimit(MerchantLimit):
    objects: models.Manager["GlobalRiskMerchantLimit"] = GlobalRiskMerchantLimitManager()  # type: ignore[misc, assignment]
