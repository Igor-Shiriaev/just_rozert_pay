...
from rozert_pay.limits.models import (
    CustomerLimit,
    LimitAlert,
    LimitCategory,
    MerchantLimit,
...
class CategoryLimitAdminBase:
    category: LimitCategory
    limit_model_class: Type[Model]
    ...
    def get_readonly_fields(
        ...
        if "category" not in readonly_fields:
    ...
    def get_form(
        ...
        **kwargs: Any,
        ...
        form = super().get_form(request, obj, change, **kwargs)  # type: ignore[misc]
        if obj is None and "category" in form.base_fields:
            form.base_fields["category"].initial = self.category
    ...
    def save_model(
        ...
        if not change and isinstance(obj, self.limit_model_class):
            obj.category = self.category
        super().save_model(request, obj, form, change)  # type: ignore[misc]


class BaseLimitAdmin(DjangoObjectActions, admin.ModelAdmin):
    changelist_actions: Iterable[str] = ("invalidate_cache_action",)
    list_display = (
        "status_colored",
        "category",
        "description",
    ...
    readonly_fields: tuple[str, ...] = ("links",)
    ...
    def invalidate_cache_action(
        self,
        request: HttpRequest,
        queryset: QuerySet[MerchantLimit | CustomerLimit],
    ) -> None:
        invalidate_limits_cache()
        self.message_user(request, _("Cache invalidated successfully."))

    invalidate_cache_action.label = _("Invalidate limits cache")  # type: ignore[attr-defined]
    invalidate_cache_action.short_description = _("Invalidate limits cache")  # type: ignore[attr-defined]
