...
from rozert_pay.payment.systems.worldpay.worldpay_client import (
    logger as worldpay_client_logger,
)
...
class TestWorldpaySystem:
    def test_deposit_validation(
        ...
        assert response.json() == {
            ...
            "request_3ds_challenge": ["This field is required."],
            "user_data": {
    ...
    def test_deposit_sends_challenge_requested_when_request_3ds_true(
        self,
        merchant_client: APIClient,
        merchant: Merchant,
        wallet_worldpay: Wallet,
        ...
        with requests_mock.Mocker() as m:
            ...
            request_payload = deepcopy(WORLDPAY_DEPOSIT_REQUEST_PAYLOAD)
            request_payload["wallet_id"] = str(wallet_worldpay.uuid)
            request_payload["request_3ds_challenge"] = True
            ...
            sent_xml = xmltodict.parse(m.request_history[0].body)
            pref = sent_xml["paymentService"]["submit"]["order"]["additional3DSData"][
                "@challengePreference"
            ]
            assert pref == "challengeRequested"

    def test_deposit_sends_no_challenge_requested_when_request_3ds_false(
        self,
        merchant_client: APIClient,
        merchant: Merchant,
        wallet_worldpay: Wallet,
        ...
        with requests_mock.Mocker() as m:
            ...
            request_payload = deepcopy(WORLDPAY_DEPOSIT_REQUEST_PAYLOAD)
            request_payload["wallet_id"] = str(wallet_worldpay.uuid)
            request_payload["request_3ds_challenge"] = False
            ...
            assert pref == "noChallengeRequested"

    def test_deposit_3ds_challenge_required_but_not_requested_logs_error(
        self,
        merchant_client: APIClient,
        merchant: Merchant,
        wallet_worldpay: Wallet,
        ...
        with requests_mock.Mocker() as m:
            ...
            with caplog.at_level(logging.ERROR, logger=worldpay_client_logger.name):
                with cm_mock_check_status_task():
                    merchant_client.post(
                        path="/api/payment/v1/worldpay/deposit/",
                        data=request_payload,
            ...
            assert (
                "3DS challenge is required for Worldpay transaction, but it was not requested"
                in caplog.text
            )

    def test_deposit_3ds_not_required_but_requested_logs_error(
        self,
        merchant_client: APIClient,
        merchant: Merchant,
        wallet_worldpay: Wallet,
        ...
        with requests_mock.Mocker() as m:
            ...
            assert (
                "3DS challenge is not required for Worldpay transaction, but it was requested"
                in caplog.text
            )
    ...
    def test_deposit_finalize_3ds_cancelled(
        ...
        """Test deposit_finalize when user cancels 3DS challenge."""
        with requests_mock.Mocker() as m:
            m.post(
                "https://secure-test.worldpay.com/jsp/merchant/xml/paymentService.jsp",
                [
