...
class IlixiumTestUtils:
    ...
    def make_deposit(
        ...
        with requests_mock.Mocker() as m:
            m.post(
                "https://prprocessing.ilixium.com/platform/ili/direct/auth",
...
class TestIlixiumFlow:
    ...
    def test_deposit(
        ...
        with requests_mock.Mocker() as m:
            m.post(
                "https://prprocessing.ilixium.com/platform/ili/direct/threedcomplete",
    ...
    def test_deposit_decline_on_auth(
        ...
        with requests_mock.Mocker() as m:
            ...
            p = xmltodict.parse(m.request_history[0].body)
            assert p["authRequest"]["customer"]["address"]["province"] == "CA-BC"
    ...
    def test_withdraw_success(self, wallet_ilixium, merchant_client: APIClient):
        ...
        with requests_mock.Mocker() as m:
            m.post(
                "https://prprocessing.ilixium.com/platform/payment/pace/api/v1/payment/create",
                json=fixtures.WITHDRAWAL_PENDING_RESPONSE,
            )
            # Optional: immediate status check endpoint
            m.post(
                "https://prprocessing.ilixium.com/platform/payment/pace/api/v1/payment/find",
                json=fixtures.WITHDRAWAL_CHECK_STATUS_RESPONSE,
            )

            resp = merchant_client.post(
                path="/api/payment/v1/ilixium/withdraw/",
                data={
                    "wallet_id": str(wallet_ilixium.uuid),
                    "amount": "1.42",
                    "currency": "CAD",
                    "customer_id": "customer",
                    "withdraw_to_account": "5252271",
                    "beneficiary_account_number": "123123",
                    "beneficiary_bank_code": "123123",
                    "beneficiary_sort_code": "123123",
                    "user_data": {
                        "language": "en",
                        "email": "test@test.com",
                        "phone": "123123123",
        ...
        assert (
            ...
            == fixtures.WITHDRAWAL_PENDING_RESPONSE["paceTransactionRef"]
        )
        assert m.request_history[0].json() == {
            "beneficiaryAccountNumber": "123123",
            "beneficiaryAddr1": "asdasdasd",
            "beneficiaryBankCode": "123123",
            "beneficiaryDob": "2000-01-01",
            "beneficiaryFirstName": "John",
            "beneficiaryLastName": "Doe",
            "beneficiaryPostcode": "123123",
            "beneficiarySortCode": "123123",
            "callbackUrl": mock.ANY,
            "merchantReference": mock.ANY,
            "paymentAmount": 1.42,
            "paymentCategory": "DISBURSEMENT",
            "paymentCurrency": "CAD",
            "paymentDate": mock.ANY,
            "paymentMerchant": "fake",
            "paymentTarget": "CANADA_EFT",
        }

    def test_withdraw_failed(self, wallet_ilixium, merchant_client: APIClient):
        CurrencyWalletFactory.create(
            wallet=wallet_ilixium,
            currency="CAD",
            hold_balance=10000,
        ...
        with requests_mock.Mocker() as m:
            m.post(
                "https://prprocessing.ilixium.com/platform/payment/pace/api/v1/payment/create",
                json=fixtures.WITHDRAWAL_FAILED_RESPONSE,
            )
