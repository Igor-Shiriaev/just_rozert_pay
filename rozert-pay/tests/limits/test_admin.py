...
class TestMerchantLimitForm:
    ...
    def test_clean_duplicate_limit_merchant_scope(self):
        ...
        WalletFactory.create(merchant=merchant)
...
class TestRiskCustomerLimitAdmin:
    ...
    def test_get_form_for_existing_limit(self, admin, admin_request):
        limit = CustomerLimitFactory.create(category=LimitCategory.RISK)
        form = admin.get_form(admin_request, obj=limit)()
        assert "category" not in form.base_fields

    def test_get_readonly_fields_excludes_category_for_new_limit(
        self,
        admin,
        admin_request,
    ):
    ...
    def test_get_readonly_fields_includes_category_for_existing_limit(
        self,
        admin,
        admin_request,
    ):
...
class TestBusinessCustomerLimitAdmin:
    ...
    def admin(self) -> BusinessCustomerLimitAdmin:
        return BusinessCustomerLimitAdmin(
            model=BusinessCustomerLimit,
...
class TestBusinessMerchantLimitAdmin:
    ...
    def admin(self) -> BusinessMerchantLimitAdmin:
        return BusinessMerchantLimitAdmin(
            model=BusinessMerchantLimit,
            admin_site=AdminSite(),
        )
...
@pytest.mark.django_db
@pytest.mark.usefixtures("disable_cache")
class TestGlobalRiskMerchantLimitAdmin:
    @pytest.fixture
    def admin(self) -> GlobalRiskMerchantLimitAdmin:
        return GlobalRiskMerchantLimitAdmin(
            model=GlobalRiskMerchantLimit,
            admin_site=AdminSite(),
        )
    ...
    def test_get_form_sets_global_risk_category_for_new_limit(
        self, admin, admin_request
    ):
        form = admin.get_form(admin_request, obj=None)()
        assert form.base_fields["category"].initial == LimitCategory.GLOBAL_RISK
        assert form.base_fields["category"].disabled is True
    ...
    def test_save_model_sets_global_risk_category(self, admin, admin_request):
        wallet = WalletFactory.create()
        data = {
            "category": LimitCategory.BUSINESS,
            "scope": MerchantLimitScope.WALLET,
            ...
            "period": LimitPeriod.ONE_HOUR,
            "active": True,
            "description": "Test global risk limit",
            "max_operations": 10,
        }
        form = MerchantLimitForm(data=data)
        assert form.is_valid()
        limit = form.save(commit=False)
        admin.save_model(admin_request, limit, form, change=False)
        assert limit.category == LimitCategory.GLOBAL_RISK

    def test_queryset_filters_by_global_risk_category(self, admin, wallet):
        MerchantLimitFactory.create(category=LimitCategory.RISK)
        MerchantLimitFactory.create(category=LimitCategory.BUSINESS)
        MerchantLimitFactory.create(
            category=LimitCategory.GLOBAL_RISK,
        ...
        assert GlobalRiskMerchantLimit.objects.count() == 1

    def test_save_model_forces_global_risk_category_even_with_different_input(
        self, admin, admin_request
    ):
        wallet = WalletFactory.create()
        data = {
    ...
    def test_readonly_fields_includes_links_and_category(self, admin):
        assert "links" in admin.readonly_fields
        assert "category" in admin.readonly_fields
