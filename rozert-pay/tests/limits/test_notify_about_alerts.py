...
class TestNotifyAboutAlerts:
    ...
    def test_multiple_critical_alerts(
        ...
        # ИСПРАВЛЕНО: Добавлен 'alert_ids' в ожидаемые kwargs
        mock_notify_slack.assert_called_once_with(
            kwargs={
                "message": "Multiple critical alerts message",
                "channel": SLACK_CHANNEL_NAME_CRITICAL_LIMITS,
                "alert_ids": [customer_critical_alert.id, merchant_critical_alert.id],
            }
        )
    ...
    def test_multiple_regular_alerts(
        ...
        mock_notify_slack.assert_called_once_with(
            kwargs={
                ...
                "channel": SLACK_CHANNEL_NAME_REGULAR_LIMITS,
                "alert_ids": [customer_regular_alert.id, merchant_regular_alert.id],
            }
        )
    ...
    def test_construct_notification_message_called_with_correct_alerts(
        ...
        mock_construct_message.return_value = "Test Notification Message"
        ...
        critical_call = next(c for c in calls if critical_alert_1 in c.args[0])
        regular_call = next(c for c in calls if regular_alert in c.args[0])

        critical_call_alerts = critical_call.args[0]
        regular_call_alerts = regular_call.args[0]
