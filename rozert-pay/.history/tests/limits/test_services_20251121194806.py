...
class TestCustomerLimits:
    ...
    def test_min_operation_amount(
        ...
        customer_limit.customer.risk_control = True
        customer_limit.customer.save()
...
class TestRiskAndBusinessLimitCategories:
    ...
    def test_risk_customer_limit_checked_when_feature_enabled(self, customer):
        ...
        with mock.patch(
            "rozert_pay.limits.services.limits.get_feature_flag_status",
            return_value=True,
        ):
            with mock.patch(
    ...
    def test_risk_customer_limit_not_checked_when_feature_disabled(self, customer):
        risk_limit = CustomerLimitFactory.create(
            customer=customer,
            category=LimitCategory.RISK,
            period=LimitPeriod.ONE_HOUR,
    ...
    def test_mixed_risk_and_business_limits_processed_correctly(
        ...
        business_customer_limit = CustomerLimitFactory.create(
            customer=customer,
            category=LimitCategory.BUSINESS,
            period=LimitPeriod.ONE_HOUR,
            max_successful_operations=1,
        ...
        assert is_declined is True
        assert len(alerts) == 1
        assert alerts[0].customer_limit == business_customer_limit
