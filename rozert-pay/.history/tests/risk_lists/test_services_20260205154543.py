...
from rozert_pay.risk_lists.const import OperationType, Scope, ValidFor
...
class TestRiskListChecker:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (Scope.GLOBAL, RiskDecision.GLOBAL_BLACKLIST),
    ...
    def test_black_list_declines_with_correct_scope_reason(
        self, scope: Scope, expected_reason: RiskDecision
    ) -> None:
        customer = CustomerFactory.create(email_encrypted="user@xxx.com")
        trx = PaymentTransactionFactory.create(customer=customer)
    ...
    @pytest.mark.parametrize(
        "scope, expected_reason",
        [
            (Scope.GLOBAL, RiskDecision.GLOBAL_GRAYLIST),
            (Scope.MERCHANT, RiskDecision.GRAYLIST),
            (Scope.WALLET, RiskDecision.GRAYLIST),
        ],
    )
