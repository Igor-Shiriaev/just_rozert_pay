...
from rozert_pay.payment.models import CurrencyWallet, PaymentTransaction, Wallet, Merchant
...
class TestConektaOxxoFlow:
    def test_deposit_flow(self, merchant: Merchant, wallet_conekta_oxxo: Wallet, merchant_client: APIClient):
        with requests_mock.Mocker() as m:
            ...
            print('4444444444444444', response.json())

            with patch('rozert_pay.payment.systems.conekta.conekta_oxxo._validate_conekta_webhook_signature', return_value=True):
                response = merchant_client.post(
                    path="/api/payment/v1/callback/conekta-oxxo/",
                    headers={
                        'DIGEST': 'Timati - U nas v klube',
                    },
                    data={
                            "type": 'order.paid',
                            "data": {
