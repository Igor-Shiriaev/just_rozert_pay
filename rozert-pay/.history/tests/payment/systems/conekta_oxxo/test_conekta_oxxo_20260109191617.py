...
class TestConektaOxxoFlow:
    ...
    def test_deposit_flow_success_without_order_id(
        self,
        wallet_conekta_oxxo: Wallet,
        merchant_client: APIClient,
        mock_final_status_validation,
        ...
        with requests_mock.Mocker() as m:
            ...
            with patch(
                ...
                # Here we parse `transaction_uuid` from metadata instead of `order_id`
                response = merchant_client.post(
                    path="/api/payment/v1/callback/conekta-oxxo/",
                    data={
                        "type": "order.paid",
    ...
    def test_credentials_action(self, merchant: Merchant):
        ...
        with requests_mock.Mocker() as m:
            ...
            creds = Wallet.objects.get()._credentials
            assert (
                    ...
                    "data": [
                        {
                            "deleted": False,
                            "id": "228",
                            "url": "https://old-webhook.rozert.cloud",
                        }
                    ],
                },
            )
...
class TestConektaWebhookSignature:
    ...
    def test_valid_signature(self, rsa_key_pair, webhook_payload):
        ...
        callback.body = json.dumps(webhook_payload)
        _validate_conekta_webhook_signature(credentials, callback)
