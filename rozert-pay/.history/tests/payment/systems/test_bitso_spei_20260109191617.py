...
def test_callback_signature_validation_success(wallet_bitso_spei: Wallet) -> None:
    ...
    key_id = "test-key"
    wallet_bitso_spei._credentials.update(
        {
            "public_keys": [
                {
                    "key_id": key_id,
    ...
    wallet_bitso_spei.save(update_fields=["credentials"])
...
def test_credentials_change_action_updates_webhooks(wallet_bitso_spei: Wallet) -> None:
    old_creds = {**wallet_bitso_spei._credentials}
    ...
    assert wallet_bitso_spei._credentials["public_keys"] == [{"key_id": "123"}]
    assert any(
