...
class TestConektaOxxoFlow:
    def test_deposit_flow(self, wallet_paypal, merchant_client):
        with requests_mock.Mocker() as m:
            m.post(
                f'https://conekta/orders/',
                json=lambda *a, **k: behaviour_container.deposit_response or DEPOSIT_RESPONSE_SUCCESS,
            )
            m.get(
                re.compile('https://conekta/orders/.*'),
                json=lambda *a, **k: behaviour_container.status_response or TRANSACTION_STATUS_SUCCESS,
            )
            m.post(
            ...
            logging.warning(f"Wallet ID: {wallet_paypal.id}")
    ...
    def test_callback_approved(self, merchant_client, wallet_paypal):
        """
