...
class TestPaypalFlow:
        ...
        with requests_mock.Mocker() as m:
            ...
            with cm_mock_check_status_task():
                response = merchant_client.post(
                    "/api/payment/v1/paypal/deposit/",
                    {
                        "type": const.TransactionType.DEPOSIT,
                        "amount": "100.00",
                        "currency": "USD",
                        "wallet_id": wallet_paypal.uuid,
            ...
            logging.warning(
                "Wallet ID",
                extra={"wallet_id": wallet_paypal.id},
            )
            ...
            assert response.json()["form"] == TRANSACTION_PAYPAL_FORM_EXAMPLE
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (entities.TransactionStatus.CHARGED_BACK, "VOIDED"),
        ],
    )
    def test_deposit_success(self, wallet_paypal, expected_status, paypal_status):
        """
    ...
    def test_withdraw_success(
        ...
        with requests_mock.Mocker() as m:
            ...
            assert trx.wallet.available_balance == Decimal("57.00")
            assert trx.wallet.frozen_balance == Decimal("43.00")
            ...
            assert trx.status == entities.TransactionStatus.SUCCESS
            trx.wallet.refresh_from_db()
            assert trx.wallet.operational_balance == Decimal("57.00")
            ...
            assert list(
                ...
                {
                    "description": "Attempt to send callback: 1 / 10",
    ...
    def test_callback_approved(
        self, merchant_client, wallet_paypal, mock_final_status_validation
    ):
        """
    ...
    def test_setup_webhooks(self, wallet_paypal):
        ...
        with requests_mock.Mocker() as m:
            ...
            PaypalClient.setup_webhooks(
                url=webhook_url, creds=creds, logger=Mock(), wallet=wallet_paypal
            )
    ...
    def test_deposit_finalize_unprocessable_entity(self, currency_wallet_paypal):
        ...
        currency_wallet_paypal.wallet.wallet.credentials = {
            "base_url": "https://api-m.sandbox.paypal.com",
            "client_id": "test_client_id",
            "client_secret": "test_secret",
            "test_mode": True,
        }
        currency_wallet_paypal.wallet.wallet.save()
...
# BEGIN paypal.form
TRANSACTION_PAYPAL_FORM_EXAMPLE = {
