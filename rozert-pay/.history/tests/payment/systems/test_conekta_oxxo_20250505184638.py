...
from rozert_pay.payment.systems.conekta.conekta_oxxo import (
    ConektaOxxoCredentials, _validate_conekta_webhook_signature)
...
class TestConektaWebhookSignature:
    ...
    def test_valid_signature(self, rsa_key_pair, webhook_payload):
        # Create test credentials with our generated public key
    ...
    def test_invalid_signature(self, rsa_key_pair, webhook_payload):
        ...
        tampered_payload["data"]["object"]["id"] = "tampered_id"
        ...
        # Validate with tampered payload - should raise ValidationError
        with pytest.raises(ValidationError, match="Invalid Conekta webhook signature"):
            _validate_conekta_webhook_signature(credentials, callback, tampered_payload)
    ...
    def test_missing_digest_header(self, rsa_key_pair, webhook_payload):
        ...
        # Create a mock callback with missing DIGEST header
        callback = Mock(spec=IncomingCallback)
        callback.headers = {}
        
        # Should raise ValidationError about missing header
    ...
    def test_non_rsa_key(self, webhook_payload):
        ...
        credentials = ConektaOxxoCredentials(
            private_key=SecretStr("test_private_key"),
            public_key=SecretStr("-----BEGIN PUBLIC KEY-----\nInvalid Key\n-----END PUBLIC KEY-----"),
            base_url="https://api.conekta.io"
        )
