...
class TestJwt:
    ...
    def test_unique_jti_per_call(self):
        ...
        token1 = generate_3ds_jwt(
            jwt_issuer, jwt_org_unit_id, jwt_mac_key, "url", "acs", "payload", "txn1",
        )
        token2 = generate_3ds_jwt(
            jwt_issuer, jwt_org_unit_id, jwt_mac_key, "url", "acs", "payload", "txn2",
        )
...
class TestWorldpayClientHelpers:
    ...
    def test_standardize_phone_number_removes_non_digits(self):
        client = MagicMock(spec=WorldpayClient)
        result = WorldpayClient._standardize_phone_number(client, "+52 55 1234 5678")
        assert result == "525512345678"

    def test_standardize_phone_number_removes_hyphens_and_spaces(self):
        client = MagicMock(spec=WorldpayClient)
        result = WorldpayClient._standardize_phone_number(client, "1-800-555-1234")
        assert result == "18005551234"

    def test_standardize_phone_number_truncates_long_numbers(self):
        client = MagicMock(spec=WorldpayClient)
        client.trx = MagicMock()
        client.trx.id = 123
        # 20 digits - should be truncated to 15
        result = WorldpayClient._standardize_phone_number(client, "12345678901234567890")
        assert result == "123456789012345"
        assert len(result) == 15

    def test_standardize_phone_number_keeps_valid_length(self):
        client = MagicMock(spec=WorldpayClient)
        result = WorldpayClient._standardize_phone_number(client, "525512345678")
        assert result == "525512345678"
