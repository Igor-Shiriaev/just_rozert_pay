...
class TestMigrateSensitiveDataCommand:
    ...
    def test_migrates_customer_card_data(self) -> None:
        ...
        customer_card = CustomerCard.objects.create(
            unique_identity="test-card-1",
            customer=customer,
            _card_data=card_data,
        )
        CustomerCard.objects.filter(id=customer_card.id).update(
            card_data_encrypted=None
        )
        ...
        customer_card.refresh_from_db()
        assert customer_card.card_data_encrypted is not None
        assert (
            customer_card.card_data_encrypted.get_secret_value() == card_data
        )

    def test_skips_customer_card_with_existing_encrypted_data(self) -> None:
        customer = CustomerFactory()
        original_data = {"card_num": "4111111111111111"}
        updated_data = {"card_num": "4000000000000002"}

        customer_card = CustomerCard.objects.create(
            unique_identity="test-card-2",
            customer=customer,
            _card_data=updated_data,
        )
        customer_card.card_data_encrypted = original_data
        customer_card.save()
        original_encrypted = (
            customer_card.card_data_encrypted.get_secret_value()  # type: ignore[union-attr]
        )
        ...
        customer_card.refresh_from_db()
        assert (
            customer_card.card_data_encrypted.get_secret_value()
            == original_encrypted
        )

    def test_customer_card_empty_data_is_not_migrated(self) -> None:
        customer = CustomerFactory()
        customer_card = CustomerCard.objects.create(
            unique_identity="test-card-3",
            customer=customer,
            _card_data={},
        )
        CustomerCard.objects.filter(id=customer_card.id).update(
        ...
        customer_card.refresh_from_db()
        assert (
            customer_card.card_data_encrypted is None
            or customer_card.card_data_encrypted.get_secret_value() is None
        )

    def test_migrates_wallet_with_credentials(self) -> None:
        """Test that command migrates wallet credentials from _credentials to credentials_encrypted."""
        wallet: Wallet = cast(Wallet, WalletFactory())
    ...
    def test_migrates_wallet_with_complex_credentials(self) -> None:
        ...
        Wallet.objects.filter(id=wallet.id).update(
            _credentials=complex_credentials,
            credentials_encrypted=None,
        )
        wallet.refresh_from_db()
    ...
    def test_migrates_multiple_wallets(self) -> None:
        ...
        for wallet, original_credentials in wallets:
            ...
            assert wallet.credentials_encrypted is not None
            assert (
                wallet.credentials_encrypted.get_secret_value() == original_credentials
            )
    ...
    def test_skips_wallet_with_existing_credentials_encrypted(self) -> None:
        ...
        wallet_to_migrate: Wallet = cast(Wallet, WalletFactory())
        credentials_to_migrate = {"should": "migrate"}
        Wallet.objects.filter(id=wallet_to_migrate.id).update(
            _credentials=credentials_to_migrate,
            credentials_encrypted=None,
        )
        wallet_to_migrate.refresh_from_db()

        wallet_to_skip: Wallet = cast(Wallet, WalletFactory())
    ...
    def test_migrates_wallet_with_empty_credentials(self) -> None:
        ...
        assert wallet.credentials_encrypted is not None
        assert wallet.credentials_encrypted.get_secret_value() is None
