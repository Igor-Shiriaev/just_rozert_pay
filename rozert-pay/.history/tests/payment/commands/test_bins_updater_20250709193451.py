...
class TestBinsUpdaterCommand:
    ...
    def test_command_handles_large_dataset(self) -> None:
        """Test that command handles large datasets efficiently."""
        large_dataset = {}
        for i in range(15000):  # More than the 10000 threshold
            large_dataset[str(100000 + i)] = {
                "br": 1,
                "bn": f"Bank {i}",
                "cc": "US",
                "type": "credit",
        ...
        try:
            ...
            assert Bank.objects.count() == 15000
            assert PaymentCardBank.objects.count() == 15000
    ...
    @patch("builtins.open")
    def test_command_handles_io_error(self, mock_open: Any) -> None:
        """Test command handles IO errors gracefully."""
        mock_open.side_effect = IOError("File read error")
        
        with pytest.raises(IOError):
            call_command("bins_updater", path="test.json")
    ...
    def test_command_with_missing_optional_fields(self) -> None:
        ...
        try:
            with pytest.raises(KeyError):  # ijson will raise KeyError for missing fields
                call_command("bins_updater", path=temp_path)
        finally:
            Path(temp_path).unlink()
