...
class TestPaymentTransactionAdmin:
    ...
    def test_links_with_full_relations(self, admin, settings):
        ...
        expected_links = [
            ...
            (
                reverse("admin:limits_riskcustomerlimit_changelist")
                + f"?id__in={customer_limit.id}",
                "Triggered Risk Customer Limits",
            ),
            (
                reverse("admin:limits_businessmerchantlimit_changelist")
                + f"?wallet__id__in={wallet_limit.id}",
                "Triggered Wallet Business Limits",
            ),
            (
                reverse("admin:limits_riskmerchantlimit_changelist")
                + f"?merchant__id__in={merchant_limit.id}",
                "Triggered Merchant Risk Limits",
            ),
            (
