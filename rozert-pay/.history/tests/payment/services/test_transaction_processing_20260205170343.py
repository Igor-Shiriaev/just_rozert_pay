...
class TestTransactionProcessingFunctions:
    def test_handle_chargeback(self, mock_balance_service: MagicMock):
        trx = PaymentTransactionFactory.create(
            ...
            customer=PaymentTransactionFactory._meta.model._meta.get_field("customer").related_model.objects.first()
            or PaymentTransactionFactory._meta.model._meta.get_field("customer").related_model.objects.create(),
        )
