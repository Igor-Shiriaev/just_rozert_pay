...
class TestTransactionProcessingFunctions:
    ...
    def test_handle_chargeback_without_customer(self, mock_balance_service: MagicMock):
        trx = PaymentTransactionFactory.create(
            status=TransactionStatus.SUCCESS,
            type=TransactionType.DEPOSIT,
            customer=None,
        ...
        assert trx.status == TransactionStatus.CHARGED_BACK
        assert trx.extra["is_chargeback_received"] is True
        assert TransactionExtraFields.IS_CUSTOMER_BLACKLISTED not in trx.extra

        assert not BlackListEntry.objects.filter(transaction=trx).exists()
