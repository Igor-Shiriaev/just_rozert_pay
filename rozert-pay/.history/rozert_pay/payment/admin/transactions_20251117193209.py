        ...
        if obj.customer:
            ...
            if triggered_customer_limit:
                customer_ids_param = ",".join(
                    str(pk) for pk in triggered_customer_limit
                )
                risk_customer_limit_ids = [
                    id
                    for id in triggered_customer_limit
                    if LimitAlert.objects.get(id=id).customer_limit.category
                    == LimitCategory.RISK
                ]
                business_customer_limit_ids = [
                    id
                    for id in triggered_customer_limit
                    if LimitAlert.objects.get(id=id).customer_limit.category
                    == LimitCategory.BUSINESS
                ]
