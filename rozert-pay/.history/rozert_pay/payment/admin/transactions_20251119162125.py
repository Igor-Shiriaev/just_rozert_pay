...
class PaymentTransactionAdmin(BaseRozertAdmin):
    ...
    def links(self, obj: models.PaymentTransaction) -> str:
        ...
        if triggered_wallet_limit_ids:
            ...
            wallet_business_limits_link = (
                reverse("admin:limits_businessmerchantlimit_changelist")
                + f"?wallet__id__in={wallet_ids_param}"
            )
            wallet_risk_limits_link = (
                reverse("admin:limits_riskmerchantlimit_changelist")
                + f"?wallet__id__in={wallet_ids_param}"
            )
            data.append(
                {
                    "link": wallet_business_limits_link,
                    "name": _("Triggered Wallet Business Limits"),
                }
            )
            data.append(
                {
                    "link": wallet_risk_limits_link,
                    "name": _("Triggered Wallet Risk Limits"),
                }
            )
        ...
        if triggered_merchant_limit_ids:
            ...
            merchant_business_limits_link = (
                reverse("admin:limits_businessmerchantlimit_changelist")
                + f"?merchant__id__in={merchant_ids_param}"
            )
            merchant_risk_limits_link = (
                reverse("admin:limits_riskmerchantlimit_changelist")
                + f"?merchant__id__in={merchant_ids_param}"
            )
            data.append(
                {
                    "link": merchant_business_limits_link,
                    "name": _("Triggered Merchant Business Limits"),
                }
            )
            data.append(
                {
                    "link": merchant_risk_limits_link,
                    "name": _("Triggered Merchant Risk Limits"),
                }
            )
                ...
                if alert.customer_limit:
                    if alert.customer_limit.category == LimitCategory.RISK:
                        ...
                        links.append(
                            (
                                link,
                                f"Risk Customer Limit: {alert.customer_limit.description}",
                            )
                        )
                    else:
                        link = reverse(
                        ...
                        links.append(
                            (
                                link,
                                f"Business Customer Limit: {alert.customer_limit.description}",
                            )
                        )
                    ...
                    if alert.merchant_limit.category == LimitCategory.RISK:
                        ...
                        links.append(
                            (
                                link,
                                f"Risk Merchant Limit: {alert.merchant_limit.description}",
                            )
                        )
                    else:
                        link = reverse(
                        ...
                        links.append(
                            (
                                link,
                                f"Business Merchant Limit: {alert.merchant_limit.description}",
                            )
                        )
            if links:
