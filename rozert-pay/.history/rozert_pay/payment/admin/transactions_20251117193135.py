    ...
    def links(self, obj: models.PaymentTransaction) -> str:
        ...
        if obj.customer:
            ...
            if triggered_customer_limit_ids:
                ...
                risk_customer_limit_ids = [id for id in triggered_customer_limit_ids if LimitAlert.objects.get(id=id).customer_limit.category == LimitCategory.RISK]
                if triggered_customer_limit_ids[0].category == LimitCategory.RISK:
                    customer_limits_link = reverse(
                        "admin:limits_riskcustomerlimit_changelist",
                        args=[triggered_customer_limit_ids[0].pk],
                    )
                else:
                    customer_limits_link = reverse(
