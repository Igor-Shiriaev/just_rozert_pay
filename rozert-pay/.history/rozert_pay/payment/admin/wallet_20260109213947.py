...
{% endfor %}
    ...
    def save(self, commit=True):
        ...
        # Get old credentials before saving
        if self.instance.pk:
            # Reload from DB to get current credentials
            old_instance = models.Wallet.objects.get(pk=self.instance.pk)
            old_creds = old_instance.credentials
        else:
            old_creds = None

        wallet = super().save(commit)
        new_creds = wallet.credentials
