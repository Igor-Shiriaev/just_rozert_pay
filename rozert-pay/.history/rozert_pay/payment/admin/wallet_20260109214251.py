...
class WalletForm(forms.ModelForm):  # pragma: no cover
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            if self.instance.credentials is not None:
                self.fields["credentials"].initial = self.instance.credentials
            else:
                self.fields["credentials"].initial = {}

        print('444444444444444', type(self.fields["credentials"].initial), self.fields["credentials"].initial)
        print('555555555', type(self.instance.credentials), self.instance.credentials)
...
{% endfor %}
    ...
    def clean(self):
        ...
        elif "credentials" in cleaned_data:
            # credentials was explicitly set to None/empty
            if not self.is_sandbox:
                raise ValidationError("Credentials cannot be empty")
            credentials = {}

        # Store credentials in cleaned_data to use in save()
        if credentials is not None:
            cleaned_data["credentials"] = credentials

        print('222222222222222', type(self.instance.credentials), self.instance.credentials)
        return cleaned_data
    ...
    def save(self, commit=True):
        ...
        print('666666666666666', type(old_creds), old_creds)

        # Set credentials before saving
        ...
        print('777777777777777', type(new_creds), new_creds)
