    ...
    def links(self, obj: models.PaymentTransaction) -> str:
        ...
        if obj.customer:
            ...
            if triggered_customer_limit:
                risk_customer_limit = [
                    limit for limit in triggered_customer_limit if limit.customer_limit.category == LimitCategory.RISK
                ]
                business_customer_limit = [
                    limit for limit in triggered_customer_limit if limit.customer_limit.category == LimitCategory.BUSINESS
                ]
                if risk_customer_limit:
                    data.append(
                        {
                            "link": reverse("admin:limits_riskcustomerlimit_changelist") + f"?id__in={','.join(str(limit.customer_limit.pk) for limit in risk_customer_limit)}",
                            "name": _("Triggered Risk Customer Limits"),
                        }
                    )
                if business_customer_limit:
                    data.append(
                        {
                            "link": reverse("admin:limits_businesscustomerlimit_changelist") + f"?id__in={','.join(str(limit.customer_limit.pk) for limit in business_customer_limit)}",
