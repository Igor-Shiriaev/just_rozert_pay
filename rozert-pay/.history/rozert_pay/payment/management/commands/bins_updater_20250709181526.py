    ...
    def handle(self, **options: Any) -> None:
        ...
        with open(path, "rb") as file:
            ...
            for bin, bin_data in ijson.kvitems(file, ""):
                ...
                self.stdout.write(
                    self.style.NOTICE(f"Created/found bank: {bin_data['bn']}")
                )
                ...
                self.stdout.write(
                    self.style.NOTICE(f"Processed bin {bin}, created: {created}")
                )
                counter += 1
                if counter % 10000 == 0:
                    self.stdout.write(
                        self.style.NOTICE(f"Processed {counter} bins")
                    )

        self.stdout.write(self.style.SUCCESS("BINs were updated successfully"))
