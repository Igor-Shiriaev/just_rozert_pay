...
class Command(BaseCommand):
    ...
    def handle(self, **options: Any) -> None:
        ...
        with open(path, 'rb') as file:
            ...
            for bin, value in ijson.kvitems(file, ''):
                bank, _ = Bank.objects.get_or_create(name=value["bn"])
                banks_by_name[value["bn"]] = bank.pk
                self.stdout.write(f"Created/found bank: {value['bn']}")
