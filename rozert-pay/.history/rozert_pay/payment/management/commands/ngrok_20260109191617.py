...
class NgrokThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        try:
            process = su.Popen(
                f"ngrok http {self.port} --log=stdout",
                shell=True,
                stdout=su.PIPE,
                stderr=su.PIPE,
                text=True,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--remove-all-webhooks",
            action="store_true",
            help="Remove all webhooks (and restore them after)",
        )
        parser.add_argument(
            "--port",
            type=int,
            default=8006,
            help="Local port to expose through ngrok (default: 8006)",
        )

    def run_ngrok_and_fill_external_host(self, port: int) -> None:
        thread = NgrokThread(port)
        thread.daemon = True
        thread.start()
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        try:
            self._handle(system, port)
        finally:
            su.call("pkill -f ngrok", shell=True)

    def _handle(self, system: str, port: int) -> None:
        self.run_ngrok_and_fill_external_host(port)
    ...
    def _handle_wallets(self, system: str, create_new: bool) -> None:
        for w in models.Wallet.objects.filter(
            ...
            writer = LogWriter()
            creds = controller.client_cls.get_credentials_from_dict(w._credentials)
