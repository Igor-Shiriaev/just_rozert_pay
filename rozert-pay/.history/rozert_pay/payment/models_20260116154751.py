...
class Wallet(BaseDjangoModel):
    ...
    _credentials: dict[str, ty.Any] = EncryptedField(db_column="credentials")
    credentials_encrypted: SecretValue[dict[str, ty.Any]] | None = EncryptedFieldV2(
        key_set_id=KeySetId.CREDENTIALS,
    ...
    def save(
        ...
        **kwargs: ty.Any,
        ...
        if self._credentials and not self.credentials_encrypted:
            self.credentials_encrypted = self._credentials

        return super().save(*args, **kwargs)

    @property
    def credentials(self) -> dict[str, ty.Any]:
        if self.credentials_encrypted is not None:
            return self.credentials_encrypted.get_secret_value()
        return self._credentials

    @credentials.setter
    def credentials(self, value: dict[str, ty.Any]) -> None:
...
class Customer(BaseDjangoModel):
    ...
    _email = models.EmailField(null=True, blank=True, db_column="email")
    _phone = models.CharField(max_length=255, null=True, blank=True, db_column="phone")
    language = models.CharField(max_length=10, null=True, blank=True)
    _extra = models.JSONField(default=dict, blank=True, db_column="extra")
    risk_control = models.BooleanField(
        default=False,
    ...
    @property
    def extra(self) -> dict[str, ty.Any]:
        if self.extra_encrypted:
            return self.extra_encrypted.get_secret_value()
        return self._extra

    @property
    def email(self) -> str | None:
        if self.email_encrypted:
            return self.email_encrypted.get_secret_value()
        return self._email

    @property
    def phone(self) -> str | None:
        if self.phone_encrypted:
            return self.phone_encrypted.get_secret_value()
        return self._phone
    ...
    def save(
        ...
        **kwargs: ty.Any,
        ...
        if self._email and self.email_encrypted is None:
            self.email_encrypted = self._email

        if self._phone and self.phone_encrypted is None:
            self.phone_encrypted = self._phone

        if self._extra and self.extra_encrypted is None:
            self.extra_encrypted = self._extra
...
class CustomerCard(BaseDjangoModel):
    ...
    _card_data: dict[str, ty.Any] | Literal[""] = EncryptedField(db_column="card_data")
    card_data_encrypted: SecretValue[dict[str, ty.Any] | Literal[""]] | None = EncryptedFieldV2(
        key_set_id=KeySetId.CARD_DATA,
        field_type=EncryptedFieldType.JSON,
        null=True,
        blank=True,
    ...
    @property
    def card_data(self) -> dict[str, ty.Any] | Literal[""]:
        if self.card_data_encrypted is not None:
            return self.card_data_encrypted.get_secret_value()
        return self._card_data

    @card_data.setter
    def card_data(self, value: dict[str, ty.Any] | Literal[""]) -> None:
        self.card_data_encrypted = value

    @property
    def card_data_entity(self) -> entities.CardData | None:
        if self.card_data_encrypted is not None:
            return entities.CardData(**self.card_data_encrypted.get_secret_value())
        if not self._card_data:
            return entities.CardData(**self._card_data)
        return None
    ...
    def save(
        ...
        **kwargs: ty.Any,
        ...
        if self._card_data and not self.card_data_encrypted:
            self.card_data_encrypted = self._card_data
