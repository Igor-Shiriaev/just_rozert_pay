...
from rozert_pay import risk_lists
...
def process_transaction(transaction_id: str) -> None:
    trx: PaymentTransaction = PaymentTransaction.objects.get(id=transaction_id)
    ...
    if _is_declined := risk_lists.handle_risk_lists(
        trx,
        controller,
    ):
        return
...
def check_status(transaction_id: int) -> None:
    with transaction.atomic():
        ...
        print('9999999999999999', locked_trx.status)
