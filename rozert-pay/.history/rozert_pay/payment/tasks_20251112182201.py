...
from rozert_pay.common import const, metrics, slack
...
def process_transaction(transaction_id: str) -> None:
    # NOTE: Don't forget to Update `select_related` and `prefetch_related` by adding entities
    # that are needed for the transaction processing
    ...
    if _is_declined := checker.check_risk_lists_and_maybe_decline_transaction(
        trx,
        controller,
    ):
        return

    if _is_declined := limits.check_limits_and_maybe_decline_transaction(  # noqa F841
        trx,
        controller,
    ):
        return
...
def send_callback(self: Task, callback_id: str) -> None:  # type: ignore[type-arg]
    ...
    if error is None:
        ...
        cb.save()
        metrics.OUTCOMING_CALLBACK_COUNT.labels(
            status="success",
            system=cb.transaction.system.type,
        ).inc()
        metrics.OUTCOMING_CALLBACKS_DURATION.labels(
            status="success",
            system=cb.transaction.system.type,
        ).observe((cb.last_attempt_at - cb.created_at).total_seconds())
        return _on_exit()
    ...
    *,
    ...
    with global_context(
        ...
        try:
            ...
            metrics.INCOMING_CALLBACK_COUNT.labels(
                status=cb.status,
                system=cb.system.type,
            ).inc()
            metrics.INCOMING_CALLBACKS_DURATION.labels(
                status=cb.status,
                system=cb.system.type,
            ).observe((timezone.now() - cb.created_at).total_seconds())
...
@app.task(queue=CeleryQueue.LOW_PRIORITY)
def check_bitso_spei_bank_codes() -> None:
    """Fetch Bitso SPEI banks and map them to PaymentCardBank BINs for Mexico (MX)."""

    def process_banks() -> None:
        """Process banks from Bitso API and map to PaymentCardBank."""
        try:
            response = requests.get("https://bitso.com/api/v3/banks/MX", timeout=10)
            ...
            if not data.get("success"):
                ...
                bitso_banks = data.get("payload", [])
        except requests.RequestException:
            logger.exception("Failed to fetch Bitso banks")  # pragma: no cover
            bitso_banks = []  # pragma: no cover

        if not bitso_banks:
            logger.info("No banks found")
            return

        for bank_data in bitso_banks:
            try:
                if not all(
                    key in bank_data
                    for key in ["code", "name", "countryCode", "isActive"]
                ):
                    logger.warning(  # pragma: no cover
                        "Skipping invalid bank data",
                        extra={"bank_data": bank_data},
                    )
                    continue  # pragma: no cover

                bitso_bank, _ = BitsoSpeiCardBank.objects.update_or_create(
                    name=bank_data["name"],
                    is_active=bank_data["isActive"],
                    country_code=bank_data["countryCode"],
                    defaults={
                        "code": bank_data["code"],
                    },
                )
                ...
                matching_banks = list(
                    PaymentCardBank.objects.filter(
                        bank__name__icontains=bank_data["name"].lower()
                    )
                )

                bitso_bank.banks.clear()
                if matching_banks:
                    bitso_bank.banks.add(*matching_banks)
                    logger.info(
                        "Linked PaymentCardBank to BitsoSpeiCardBank",
                        extra={
                            "matched_count": len(matching_banks),
                            "BitsoSpeiCardBank name": bitso_bank.name,
                            "BitsoSpeiCardBank code": bitso_bank.code,
                        },
                    )
                else:
                    logger.info(
    ...
    try:
        process_banks()
        logger.info("Successfully processed Bitso banks and BIN relations")
    except Exception:
        logger.exception("Error processing Bitso banks")
