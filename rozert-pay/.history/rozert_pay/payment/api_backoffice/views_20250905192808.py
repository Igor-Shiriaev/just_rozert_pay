...
class CabinetAlertViewSet(viewsets.GenericViewSet):
    ...
    def acknowledge(self, request: Request, pk: Optional[int] = None) -> Response:
        ...
        try:
            ...
            LogEntry.objects.log_actions(
                user_id=user.id,
                queryset=LimitAlert.objects.filter(pk=pk),
                action_flag=CHANGE,
                change_message=f"Limit alert acknowledged by {user.id}",
            )
            AuditLogEntry.objects.log_create(
    instance=alert,
    action=AuditLogEntry.Action.UPDATE,            # or .ACCESS if you prefer
    changes_text=f"Limit alert acknowledged by {user.id}",
    additional_data={"acknowledged_by": user.id},  # optional, structured payload
    actor=user,                                     # actor gets recorded (also set via middleware)
    force_log=True,                                 # create entry even when no field diffs
)
    ...
    def acknowledge_all(self, request: Request) -> Response:
        ...
        for alert in alerts_to_acknowledge:
            LogEntry.objects.log_actions(
                user_id=user.id,
                queryset=LimitAlert.objects.filter(pk=alert.pk),
                action_flag=CHANGE,
                change_message=f"Limit alert acknowledged by {user.id}",
            )
