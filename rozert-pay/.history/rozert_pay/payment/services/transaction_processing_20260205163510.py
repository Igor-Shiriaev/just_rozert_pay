...
from rozert_pay.common.const import EventType, TransactionExtraFields, TransactionStatus
...
@track_duration("transaction_processing.schedule_periodic_status_checks")
...
@track_duration("transaction_processing.handle_chargeback")
def handle_chargeback(
    ...
    assert not trx.extra.get(TransactionExtraFields.IS_CHARGEBACK_RECEIVED)
    # Lock wallet explicitly to ensure safe balance operations
    wallet = CurrencyWallet.objects.select_for_update().get(id=trx.wallet_id)

    trx.status = entities.TransactionStatus.CHARGED_BACK
    trx.extra[TransactionExtraFields.IS_CHARGEBACK_RECEIVED] = True
    trx.save()

    balance_tx_record = BalanceUpdateService.update_balance(
        BalanceUpdateDTO(
            ...
            description=f"Chargeback received for transaction {trx.id}",
        )
    )

    add_customer_to_blacklist_by_trx(trx, "Chargeback received")
...
@track_duration("transaction_processing.handle_refund")
def handle_refund(
    trx: "db_services.LockedTransaction",
    refund_amount: Money,
) -> None:
    ...
    assert trx.status == entities.TransactionStatus.SUCCESS
    assert trx.type == const.TransactionType.DEPOSIT
    assert not trx.extra.get(TransactionExtraFields.IS_REFUND_RECEIVED)
    assert trx.currency == refund_amount.currency
    # Lock wallet explicitly
    wallet = CurrencyWallet.objects.select_for_update().get(id=trx.wallet_id)

    trx.status = entities.TransactionStatus.REFUNDED
    trx.extra[TransactionExtraFields.IS_REFUND_RECEIVED] = True

    trx_refund_amount_total = trx.extra.get(
        TransactionExtraFields.REFUNDED_AMOUNT, Decimal(0)
    )
    trx_refund_amount_total += refund_amount.value
    assert trx.amount - trx_refund_amount_total >= Decimal(0)
    trx.extra[TransactionExtraFields.REFUNDED_AMOUNT] = str(trx_refund_amount_total)
    trx.save()
    ...
    balance_tx_record = BalanceUpdateService.update_balance(
        BalanceUpdateDTO(
            ...
            payment_transaction=trx,
            description=f"Refund for transaction {trx.id} with amount {refund_amount.value}",
            initiator=InitiatorType.SYSTEM,
        )
    )
    ...
    event_logs.create_transaction_log(
        trx_id=trx.id,
        event_type=EventType.PAYOUT_REFUND,
        description="Refund processed, balance updated.",
        extra={
            "balance_transaction_id": str(balance_tx_record.id),
            "operational_after": str(balance_tx_record.operational_after),
        },
...
@track_duration("transaction_processing.handle_chargeback_reversal")
def handle_chargeback_reversal(
    trx: "db_services.LockedTransaction",
) -> None:
    assert connection.in_atomic_block
    assert trx.status == entities.TransactionStatus.CHARGED_BACK
    assert trx.type == const.TransactionType.DEPOSIT
    assert trx.extra.get(TransactionExtraFields.IS_CHARGEBACK_RECEIVED)
    assert not trx.extra.get(TransactionExtraFields.IS_CHARGEBACK_REVERSAL_RECEIVED)
    # Lock wallet explicitly
    wallet = CurrencyWallet.objects.select_for_update().get(id=trx.wallet_id)

    trx.status = entities.TransactionStatus.SUCCESS
    trx.extra[TransactionExtraFields.IS_CHARGEBACK_REVERSAL_RECEIVED] = True
    trx.save()

    # A chargeback reversal is a credit to the merchant. MANUAL_ADJUSTMENT is used for this.
    balance_tx_record = BalanceUpdateService.update_balance(
        BalanceUpdateDTO(
            currency_wallet=wallet,
            event_type=BalanceTransactionType.MANUAL_ADJUSTMENT,
            amount=Money(trx.amount.copy_abs(), trx.currency),
            payment_transaction=trx,
            description=f"Chargeback reversal for transaction {trx.id}",
            initiator=InitiatorType.SYSTEM,
        )
    )
    ...
    event_logs.create_transaction_log(
        trx_id=trx.id,
        event_type=EventType.CHARGE_BACK_REVERSAL,
        description="Chargeback reversal received, balance updated.",
        extra={
            "balance_transaction_id": str(balance_tx_record.id),
            "operational_after": str(balance_tx_record.operational_after),
        },
...
@track_duration("transaction_processing.validate_signature_for_callback")
...
@transaction.atomic
@track_duration("transaction_processing.revert_to_pending")
def revert_to_pending(
    ...
    event_to_fire: BalanceTransactionType | None = None

    match (trx.status, trx.type):
        case (TransactionStatus.SUCCESS, const.TransactionType.DEPOSIT):
            event_to_fire = BalanceTransactionType.CHARGE_BACK
        case (TransactionStatus.FAILED, const.TransactionType.DEPOSIT):
            pass
        case (TransactionStatus.SUCCESS, const.TransactionType.WITHDRAWAL):
            event_to_fire = BalanceTransactionType.SETTLEMENT_REVERSAL
        case (TransactionStatus.FAILED, const.TransactionType.WITHDRAWAL):
            event_to_fire = BalanceTransactionType.SETTLEMENT_REQUEST

    balance_tx_record = None
    if event_to_fire:
        balance_tx_record = BalanceUpdateService.update_balance(
            BalanceUpdateDTO(
                currency_wallet=trx.wallet,
                event_type=event_to_fire,
                amount=Money(trx.amount.copy_abs(), trx.currency),
                payment_transaction=trx,
                description=f"Revert to pending from status {trx.status}",
                initiator=InitiatorType.SYSTEM,
            )
        )
    ...
    extra_log = {}
    if balance_tx_record:
        extra_log = {
            "balance_transaction_id": str(balance_tx_record.id),
            "operational_after": str(balance_tx_record.operational_after),
            "frozen_after": str(balance_tx_record.frozen_after),
        }
...
@track_duration("transaction_processing.save_id_in_payment_system")
...
@track_duration("transaction_processing.stop_periodic_status_checks")
...
class TransactionPeriodicCheckService:
    @classmethod
    @track_duration("TransactionPeriodicCheckService._get_next_check_datetime")
    ...
    @classmethod
    @track_duration("TransactionPeriodicCheckService._on_check_scheduled")
