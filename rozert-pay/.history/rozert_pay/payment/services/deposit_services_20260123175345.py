...
@track_duration("deposit_services.create_deposit_instruction")
def create_deposit_instruction(
    *,
    trx: "db_services.LockedTransaction",
...
@track_duration("deposit_services.initiate_deposit")
def initiate_deposit(
    ...
    *,
    ...
    with transaction.atomic():
        ...
        locked_trx.save()
        # Don't schedule check immediately if customer redirect is required (e.g., 3DS)
        # The check should happen after customer returns from redirect
        should_schedule_immediately = (
            schedule_check_immediately and not response.customer_redirect_form_data
        )
        transaction_processing.schedule_periodic_status_checks(
            trx=locked_trx,
            schedule_check_immediately=should_schedule_immediately,
        )
...
@track_duration("deposit_services.get_return_url")
...
@track_duration("deposit_services.handle_deposit_redirect")
