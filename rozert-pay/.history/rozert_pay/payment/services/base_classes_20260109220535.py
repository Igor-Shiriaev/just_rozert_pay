...
class BasePaymentClient(ty.Generic[T_Credentials]):
    ...
    @classmethod
    def get_credentials_from_dict(cls, data: dict[str, Any] | str | None) -> T_Credentials:
    ...
    def creds(self) -> T_Credentials:
        return self.get_credentials_from_dict(self.trx.wallet.wallet.credentials)
    ...
    def setup_webhooks(
        ...
        *,
        ...
        url = url or incoming_callbacks.get_rozert_callback_url(
            system=system or wallet.system,
        )
        if only_rozert_urls:
