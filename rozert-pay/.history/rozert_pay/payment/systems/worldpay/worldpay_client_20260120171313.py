...
class WorldpayCreds(BaseModel):
    ...
    # API Key: 8225e9be-2070-44f8-9a4b-93450ae0323c
    # API ID: 691b2bea6a045063eabf6816
    # Org Unit ID: 691b2bead9a89a14402a3dd7
...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def deposit(self) -> entities.PaymentClientDepositResponse:
        ...
        xml_payload: str = generate_worldpay_xml(
            self._build_deposit_payload(card_data, self.trx.user_data, browser_data)
        )
        ...
        if "challengeRequired" in order_status:
            ...
            iframe_height, iframe_width = self._get_iframe_size(
                browser_data, three_ds_challenge_details
            )
    ...
    def deposit_finalize(self) -> entities.PaymentClientDepositFinalizeResponse:
        ...
        For 3DS2, after Cardinal Commerce challenge, we send a second request
        to get the final transaction status.
        """
    ...
    def _get_error_details(
        ...
        if payload_type in {
            ...
            if order_entity := reply_entity.get("orderStatus"):
                ...
                error_code: str | None = reply_entity.get("error", {}).get("@code")
                error_reason: str | None = reply_entity.get("error", {}).get("#text")
    ...
    @staticmethod
    def _extract_cookies_from_response(resp: requests.Response) -> dict[str, str]:
        cookies: dict[str, str] = {}
        header_values: list[str] = []
        raw_headers = getattr(resp, "raw", None)
        if raw_headers is not None and getattr(raw_headers, "headers", None) is not None:
            getlist = getattr(raw_headers.headers, "getlist", None)
            if callable(getlist):
                header_values = list(getlist("Set-Cookie"))
