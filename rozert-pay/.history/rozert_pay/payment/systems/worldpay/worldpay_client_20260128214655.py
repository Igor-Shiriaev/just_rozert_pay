...
from rozert_pay.payment.services import base_classes, db_services, deposit_services, event_logs
...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def deposit_finalize(self) -> entities.PaymentClientDepositFinalizeResponse:
        ...
        if error_code:
            # Error code "5" = "Could not find payment for order" - user hasn't completed 3DS yet
            if error_code == "5":
                event_logs.create_transaction_log(
                    trx_id=self.trx.id,
                    event_type=const.EventType.INFO,
                    description="Payment not found - user hasn't completed 3DS yet",
                    extra={"error_code": error_code, "error_reason": error_reason},
                )
