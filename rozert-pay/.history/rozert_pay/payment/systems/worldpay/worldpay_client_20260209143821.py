...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def deposit(self) -> entities.PaymentClientDepositResponse:
        ...
        trx_extra = cast(dict[WorldpayTransactionExtraFields, str | bool], self.trx.extra)
        assert trx_extra.get(WorldpayTransactionExtraFields.SESSION_ID)
        assert trx_extra.get(WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE)
        ...
        xml_payload: str = generate_worldpay_xml(
            self._build_deposit_payload(
                card_data,
                self.trx.user_data,
                browser_data,
                request_3ds_challenge=trx_extra[WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE],
            )
        )
        ...
        if "challengeRequired" in order_status:
            if not trx_extra[WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE]:
                logger.error(
                    "3DS challenge is required for Worldpay transaction, but 3DS challenge was not requested",
                    extra={
                        "trx_id": self.trx.id,
        ...
        if trx_extra[WorldpayTransactionExtraFields.REQUEST_3DS_CHALLENGE]:
            logger.error(
                "3DS challenge is NOT required for Worldpay transaction, although it was requested",
                extra={
                    "trx_id": self.trx.id,
                    "order_code": order_status["@orderCode"],
                },
