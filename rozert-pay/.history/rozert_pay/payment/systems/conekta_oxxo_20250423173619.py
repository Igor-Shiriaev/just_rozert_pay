...
class ConektaOxxoClient(base_classes.BasePaymentClient[ConektaOxxoCredentials]):
    ...
    def _get_transaction_status(self) -> entities.RemoteTransactionStatus:trx = PaymentTransaction.objects.get(uuid=trx_uuid)
        creds = self.get_creds(trx)
        response = self._make_request(
            url=f'/orders/{trx.id_in_payment_system}/',
            creds=creds,
            method='get',
        )
        response = self.make_request(
            path=f'/orders/{trx.id_in_payment_system}/',
            method="get",
        )
            ...
            'charges': [
                {
                    'payment_method': {
                        'type': 'oxxo_cash',
                        'expires_at': int((timezone.now() + datetime.timedelta(hours=24)).timestamp()),
                    }
                },
            ],
        }
