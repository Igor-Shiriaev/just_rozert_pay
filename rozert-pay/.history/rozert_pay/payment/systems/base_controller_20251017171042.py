...
class PaymentSystemController(Generic[T_Client, T_SandboxClient]):
    ...
    def __init__(
        ...
        *,
        ...
        self.allow_transition_success_to_failed_for = allow_transition_success_to_failed_for
        self.allow_transition_failed_to_success_for = allow_transition_failed_to_success_for
    ...
    def sync_remote_status_with_transaction(
        ...
        *,
        ...
        if trx.status == TransactionStatus.SUCCESS:
            # Successfull deposit - increase balance
            if trx.type == TransactionType.DEPOSIT:
                balance_before = trx.wallet.balance
                ...
                self.create_log(
                    ...
                    extra={
                        ...
                        "balance_after": trx.wallet.balance,
                        "wallet": trx.wallet_id,
                    },
                )
                return
            ...
            # Successfull payout - decrease hold balance
                ...
                hold_balance_before = trx.wallet.hold_balance
                trx.wallet.hold_balance -= trx.amount
                trx.wallet.save()

                if trx.wallet.hold_balance < 0:
                    logger.error(
                        "Hold balance is negative",
                        extra={
                            "trx_id": trx_id,
                            "hold_balance": trx.wallet.hold_balance,
                        },
                    )
                ...
                self.create_log(
                    ...
                    extra={
                        ...
                        "hold_balance_after": trx.wallet.hold_balance,
                        "wallet": trx.wallet_id,
                    },
                )
                return
    ...
    def parse_callback(
        ...
        with transaction.atomic():
            ...
            try:
                ...
                print('44444444444444444444', cb.transaction.status == TransactionStatus.FAILED, remote_status.operation_status == TransactionStatus.SUCCESS, self.allow_transition_failed_to_success_for)
