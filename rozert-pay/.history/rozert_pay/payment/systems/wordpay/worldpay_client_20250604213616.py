...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def deposit(self) -> entities.PaymentClientDepositResponse:
        ...
        xml_payload: str = self._build_xml_payload(self._build_payload(card_data, self.trx.user_data))
    ...
    def _build_payload(self, card_data: entities.CardData, user_data: entities.UserData) -> str:
        return {
            "paymentService": {
                "@version": "1.4",
    ...
    def _get_withdrawal_payment_method(self, card_data: entities.CardData) -> str:
    ...
    def withdraw(self) -> entities.PaymentClientWithdrawResponse:
        ...
        payload = self._build_payload(card_data, self.trx.user_data)
        # Yes, `REFUND` is correct here for withdrawal
        payload["paymentService"]["submit"]["order"]["paymentDetails"]["@action"] = "REFUND"

        xml_payload: str = self._build_xml_payload(payload)
