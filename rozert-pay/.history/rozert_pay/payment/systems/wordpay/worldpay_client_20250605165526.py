...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def _get_transaction_status(self) -> entities.RemoteTransactionStatus:
        ...
        foreign_status = parsed_data["paymentService"]["reply"]["orderStatus"]["@orderCode"]
        ...
        if status == entities.TransactionStatus.FAILED:
            decline_code = parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["ISO8583ReturnCode"]
            decline_reason = parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["ISO8583ReturnMessage"]
        else:
            decline_code = None
            decline_reason = None
            amount_emtity = parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["balance"]["amount"]["@value"]
            remote_amount = Money(
                Decimal(parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["amount"]["@value"]),
                parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["amount"]["@currencyCode"],
            )
