...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def _get_transaction_status(self) -> entities.RemoteTransactionStatus:
        ...
        if status == entities.TransactionStatus.FAILED:
            decline_code = parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["ISO8583ReturnCode"]["@code"]
            decline_reason = parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["ISO8583ReturnCode"]["@description"]
            remote_amount = None
        else:
            decline_code = None
            decline_reason = None
