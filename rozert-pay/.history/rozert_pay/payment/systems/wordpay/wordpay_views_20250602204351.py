...
class WordpayDepositSerializer(  # type: ignore[misc]
    ...
    serializers.user_data_serializer_mixin_factory(  # type: ignore[misc]
        "WordpayUserData", required_fields=["email", "language"]
    ),
):
...
class WordpayCardTokenWithdrawSerializer(  # type: ignore[misc]
    serializers.WithdrawalTransactionRequestSerializer,
...
""",
    ...
    def withdraw_by_token(self, request: Request) -> Response:
        return self._generic_withdraw(
            request.data, serializer_class=WordpayCardTokenWithdrawSerializer
        )
