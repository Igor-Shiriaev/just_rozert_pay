...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def _build_withdraw_payload(self, card_data: entities.CardData, payment_details_type: Literal["CARD-SSL", "ECMC-SSL"]) -> str:
        return {
            "paymentService": {
                "@version": "1.4",
                "@merchantCode": self.creds.merchant_code.get_secret_value(),
    ...
    def _get_withdrawal_payment_method(self, card_data: entities.CardData) -> Literal["CARD-SSL", "ECMC-SSL"]:
    ...
    def withdraw(self) -> entities.PaymentClientWithdrawResponse:
        ...
        payload = self._build_deposit_payload(card_data, self.trx.user_data)
        # Yes, `REFUND` is correct here.
        payload["paymentService"]["submit"]["order"]["paymentDetails"]["@action"] = "REFUND"

        xml_payload: str = self._build_withdraw_payload(payload)
