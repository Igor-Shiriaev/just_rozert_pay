...
class WorldpayController(
    ...
    def _parse_callback(self, cb: IncomingCallback) -> RemoteTransactionStatus:
        ...
        client = self.client_cls(trx_id=transaction_id)

        remote_transaction_status: entities.RemoteTransactionStatus | Error = (
            self.client_cls(trx_id=transaction_id).get_transaction_status()
        )
