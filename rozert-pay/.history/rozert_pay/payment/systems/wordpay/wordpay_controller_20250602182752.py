...
class WordpayController(
    base_controller.PaymentSystemController[WordpayClient, WordpaySandboxClient]
):
    client_cls = WordpayClient
    sandbox_client_cls = WordpaySandboxClient

    def _run_deposit(
        self, trx_id: int, client: WordpaySandboxClient | WordpayClient
    ) -> None:
    ...
    def _run_withdraw(
        self, trx: PaymentTransaction, client: WordpaySandboxClient | WordpayClient
    ) -> None:
    ...
    def _is_callback_signature_valid(self, cb: IncomingCallback) -> bool:
        ...
        def signature_for_creds_callable(creds: WordpayCreds) -> str:
    ...
    def handle_redirect(
        ...
        if trx.extra.get(APPEX_PARES):
            # Wordpay fails transaction in case we send duplicate deposit finalize.
...
appex_controller = WordpayController(
