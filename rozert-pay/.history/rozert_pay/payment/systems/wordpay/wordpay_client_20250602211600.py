...
class WordpayClient(base_classes.BasePaymentClient[WordpayCreds]):
    ...
    def _build_deposit_xml_payload(self, card_data: entities.CardData, user_data: entities.UserData):
        payment_dict = {
            "paymentService": {
                "@version": "1.4",
                ...
                "submit": {
                    "order": {
                        ...
                        "paymentDetails": {
                            "CARD-SSL": {
                                ...
                                "cardAddress": {
                                    "address": {
                                        ...
                                        "address2": user_data.address,
                                        "address3": user_data.address,
                            ...
                            "session": {
                                "@shopperIPAddress": self.trx.ip_address,
                                "@id": "0215ui8ib1",
                            },
                        },
                        "shopper": {
                            "shopperEmailAddress": "ashopper@myprovider.com",
                            "browser": {
        ...
        # 2) Unparse to XML (adds <?xml version="1.0" encoding="UTF-8"?>):
        ...
        xml_str = xml_bytes.decode("utf-8")  # xmltodict returns bytes when encoding is set

        # 3) Insert the DOCTYPE right after the XML declaration:
        doctype_line = (
            '<!DOCTYPE paymentService PUBLIC '
            '"-//Worldpay//DTD Worldpay PaymentService v1//EN" '
            '"http://dtd.worldpay.com/paymentService_v1.dtd">'
        )
        # Split once on the first newline, so we can inject DOCTYPE below the <?xml ... ?> line
        parts = xml_str.split("\n", 1)
        xml_with_doctype = f"{parts[0]}\n{doctype_line}\n{parts[1]}"
