...
class WorldpayClient(base_classes.BasePaymentClient[WorldpayCreds]):
    ...
    def _get_transaction_status(self) -> entities.RemoteTransactionStatus:
        ...
        if parsed_data["paymentService"].get("reply"):
            ...
            decline_reason = parsed_data["paymentService"]["reply"]["orderStatus"]["payment"]["ISO8583ReturnCode"]
            remote_amount = None
        elif parsed_data["paymentService"].get("notify"):
            foreign_status = parsed_data["paymentService"]["notify"]["orderStatusEvent"]["payment"]["lastEvent"]
            decline_code = None
            decline_reason = None
            remote_amount = None
        else:
