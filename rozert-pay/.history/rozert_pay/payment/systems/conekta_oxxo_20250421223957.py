    ...
    def _make_request_cls(
        ...
        *,
        ...
        resp = self.session.request(
            method=method,
            url=f'{creds.base_url}{url}',
            json=payload,
        )
        self.log_and_check_response(resp, ignore_response_predicate=lambda r: True)


        url = f"{creds.base_url}{path}"
        headers = {
            'authorization': f'Bearer {creds.private_key.get_secret_value()}',
