...
from rozert_pay.payment.services.merchant_status import StatusType, log_status_change
...
class Merchant(BaseDjangoModel):
    ...
    def save(
        ...
        **kwargs: ty.Any,
        ...
        status_changes: dict[StatusType, tuple[str | None, str | None]] = {}
        if self.pk:
            previous = (
                Merchant.objects.filter(pk=self.pk)
                .values("operational_status", "risk_status")
                .first()
            )
            if previous:
                if previous["operational_status"] != self.operational_status:
                    status_changes["operational"] = (
                        ty.cast(str | None, previous["operational_status"]),
                        self.operational_status,
                    )
                if previous["risk_status"] != self.risk_status:
                    status_changes["risk"] = (
                        ty.cast(str | None, previous["risk_status"]),
                        self.risk_status,
                    )
        ...
        if not status_change_reason:
            raise ValueError(
                "status_change_reason must be provided when merchant status changes"
            )

        for status_type, (old_value, new_value) in status_changes.items():
            reason = status_change_reason.get(status_type)
            if reason is None:
                raise ValueError(
                    f"status_change_reason missing entry for {status_type} status change"
                )
            log_status_change(
                merchant=self,
                ...
                to_status=new_value,
                reason_code=ty.cast(str | None, reason.get("reason_code")),
                comment=ty.cast(str | None, reason.get("comment")),
