...
def blacklist_for_merchant_by_trx(
    ...
    assert not trx.extra.get(TransactionExtraFields.IS_CUSTOMER_ADDED_TO_BLACKLIST)

    trx.extra[TransactionExtraFields.IS_CUSTOMER_ADDED_TO_BLACKLIST] = True
    trx.save()
    ...
    RiskListHistory.objects.create(
        blacklist_entry=black_list_entry,
        changed_by_repr="System",
        change_type=RiskListHistory.ChangeType.ADDED,
        reason=reason,
    )
    ...
    event_logs.create_transaction_log(
        ...
        description=(
            f"Customer <uuid:{customer.uuid}> has been blacklisted "
