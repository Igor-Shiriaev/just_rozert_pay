...
def add_customer_to_blacklist_by_trx(
    ...
    existent_black_list_entry = BlackListEntry.objects.filter(customer=customer).first()
    if existent_black_list_entry:
        logger.error(
            "Customer was able to initiate a transaction despite being already blacklisted",
            extra={
                "customer_id": customer.id,
                "merchant_id": merchant.id,
                "transaction_id": trx.id,
                "black_list_entry_id": existent_black_list_entry.id,
                "existent_black_list_entry_reason": existent_black_list_entry.reason,
            },
        )
        event_logs.create_transaction_log(
            trx_id=trx.id,
            event_type=EventType.INFO,
            description=("Customer is already blacklisted"),
            extra={
                "customer_id": customer.id,
                "merchant_id": merchant.id,
                "black_list_entry_id": existent_black_list_entry.id,
