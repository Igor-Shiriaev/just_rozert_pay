...
from rozert_pay.risk_lists.const import ListType, OperationType, ParticipationType
...
_PARTICIPATION_PRIORITY: Mapping[ParticipationType, int] = {
    ParticipationType.WALLET: 0,
    ParticipationType.MERCHANT: 1,
    ParticipationType.GLOBAL: 2,
}
...
def _process_transaction(trx: payment_models.PaymentTransaction) -> RiskCheckResult:
    ...
    for entry in entries:
        ...
        participation_type = ParticipationType(entry.scope)
        ...
        if list_type == ListType.WHITE:
            logger.info(
                ...
                f"list_type={list_type.value}, participation={participation_type.value}",
                extra={
        ...
        if list_type == ListType.BLACK:
            decision = (
                ...
                if participation_type == ParticipationType.GLOBAL
...
def _sort_key(entry: risk_models.RiskListEntry) -> tuple[int, int, int]:
    ...
    pt = ParticipationType(entry.scope)
    return (
...
def _get_active_entries(
    ...
    scope_q = Q(participation_type=ParticipationType.GLOBAL)
    ...
    if wallet_id is not None:
        scope_q |= Q(
            participation_type=ParticipationType.WALLET,
            wallet_id=wallet_id,
        )
    t0 = perf_counter()
    entries = list(
