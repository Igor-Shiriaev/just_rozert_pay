...
class RiskListEntry(models.Model):
    ...
    wallet = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Link to a wallet, if a scope is Wallet."),
    )
    merchant = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Link to a merchant, if a scope is Merchant."),
    )
    transaction = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Transaction which triggered the list entry."),
    )

    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    valid_for = models.CharField(max_length=10, choices=ValidFor.choices,)
    ...
    customer_wallet_id = models.CharField(
        verbose_name=_("Customer Wallet ID"),
        max_length=255,
        null=True,
        blank=True,
    )
    ...
    def clean(self) -> None:
        ...
        if self.scope == Scope.GLOBAL:
            if self.merchant:
                errors.setdefault("merchant", []).append(
                    _("For GLOBAL scope, 'merchant' must be empty.")
                )
            if self.wallet:
                errors.setdefault("wallet", []).append(
                    _("For GLOBAL scope, 'wallet' must be empty.")
                )
            if self.customer:
                errors.setdefault("customer", []).append(
                    _("For GLOBAL scope, 'customer' must be empty")
                )

        elif self.scope == Scope.MERCHANT:
            if not self.merchant:
                errors.setdefault("merchant", []).append(
                    _("For MERCHANT scope, 'merchant' must be selected.")
                )
            if self.wallet:
                errors.setdefault("wallet", []).append(
                    _("For MERCHANT scope, 'wallet' must be empty.")
                )
            if not self.customer:
                errors.setdefault("customer", []).append(
                    _("Customer is required for Merchant scope.")
                )

        elif self.scope == Scope.WALLET:
            if not self.wallet:
                errors.setdefault("wallet", []).append(
                    _("For WALLET scope, 'wallet' must be selected.")
                )
            if self.merchant:
                errors.setdefault("merchant", []).append(
                    _("For WALLET scope, 'merchant' must be empty.")
                )
            if not self.customer:
                errors.setdefault("customer", []).append(
                    _("Customer is required for Wallet scope.")
                )
        ...
        if self.scope == Scope.GLOBAL and self.list_type in [
            ListType.BLACK,
            ListType.GRAY,
        ]:
            field_count = len(
