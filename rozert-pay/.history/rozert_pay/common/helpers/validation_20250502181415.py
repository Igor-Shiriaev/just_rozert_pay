...
def validate_mexican_curp(value: str) -> str:
    curp_birthdate = datetime.strptime(value[4:10], '%y%m%d').date()
...
def validate_clabe(value: str) -> str:
    if len(value) != 18:
        raise serializers.ValidationError('CLABE must be 18 digits long')

    check_digit_to_verify = calculate_clabe_check_digit(value[:17])
    check_digit = int(value[17])
    if check_digit_to_verify != check_digit:
        raise serializers.ValidationError('Invalid CLABE')