...
def disable_cache_for_thread() -> ty.Iterator[None]:
...
def _memory_caching(
    ...
    if val := _cache.get(key):
        if val["expires"] > time.time():
            ...
            last_invalidation_request_raw = cache.get(f"cache:last_invalidation_request:{key}")

            # If no last_invalidation_request in cache it means we can use cached value
            last_invalidation_request = float(last_invalidation_request_raw) if last_invalidation_request_raw else 0
...
def memory_cache_invalidate(key: CacheKey) -> None:
    # Use Django's cache framework instead of direct Redis access
    # Cache TTL should be enough for all processes to read this value and invalidate it's local caches.
