...
class AdminLogFormMixin:
    ...
    def _log_admin_action(
        ...
        if action_flag == CHANGE and previous_state:
            # Get list of changed field names for Django's standard format
            changed_field_names = [
                field for field in current_state
                if previous_state.get(field) != current_state[field]
            ]
            
            # Create Django's standard change message format
            if changed_field_names:
                change_message = json.dumps([{"changed": {"fields": changed_field_names}}])
            else:
                change_message = json.dumps([{"changed": {"fields": []}}])
        else:
            # For additions, use Django's standard format
