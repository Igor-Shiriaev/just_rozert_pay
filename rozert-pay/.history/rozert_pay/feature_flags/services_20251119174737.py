...
def update_feature_flag_status(*, name: FeatureFlagName, status: bool) -> None:
    ...
    memory_cache_invalidate(FEATURE_FLAGS_CACHE_KEY.format(name=name))  # type: ignore[arg-type]
...
def get_feature_flag_status(*, name: FeatureFlagName) -> bool:
    ...
    return memory_cache_get_set(
        key=FEATURE_FLAGS_CACHE_KEY.format(name=name),  # type: ignore[arg-type]
        tp=bool,
