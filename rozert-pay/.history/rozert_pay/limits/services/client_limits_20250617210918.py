...
def _check_customer_limit(
    ...
    if limit.max_successful_operations:
        successful_count = customer_transactions.filter(
            status="SUCCESS"
        ).count()
    ...
    if limit.max_failed_operations:
        failed_count = customer_transactions.filter(
            status="FAILED"
        ).count()
    ...
    if limit.total_successful_amount:
        total_amount = customer_transactions.filter(
            status="SUCCESS"
...
def _check_merchant_limit(
    ...
    if limit.scope == limit_models.MerchantLimitScope.MERCHANT:
        transactions = db_services.PaymentTransaction.objects.filter(
            wallet__wallet__merchant_id=limit.merchant_id,
            created_at__gte=period_start
        )
    else:  # WALLET scope
        transactions = db_services.PaymentTransaction.objects.filter(
            wallet_id=limit.wallet_id,
            created_at__gte=period_start
        )
