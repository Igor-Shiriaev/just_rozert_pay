...
from rozert_pay.limits.services.utils import FilteredOutLimit, construct_notification_message
...
def _should_check_customer_limit(
    ...
    does_limit_belong_to_customer = bool(
        trx.customer and trx.customer == limit.customer
    )
    if not does_limit_belong_to_customer:
        return False, "limit does not belong to customer"

    assert trx.customer is not None

    if limit.category == LimitCategory.BUSINESS:
        if not (trx.customer.risk_control and is_customer_in_gray_list):
            return True, None
        return False, "business limit but customer has risk_control and the limit is in gray_list"

    if limit.category == LimitCategory.RISK:
        if trx.customer.risk_control and is_customer_in_gray_list:
            return True, None
        return False, "risk limit but customer lacks risk_control or the limit is not in gray_list"
...
def _should_check_merchant_limit(
    ...
    if not does_limit_belong_to_merchant and not does_limit_belong_to_wallet:
        return False, f"limit does not belong to merchant/wallet (scope={limit.scope})"
    ...
    return False, f"risk/global_risk limit but risk_control not enabled (category={limit.category})"
...
def _process_transaction_limits(
    ...
    for limit in active_limits:
        ...
        if isinstance(limit, limit_models.CustomerLimit):
            if not trx.customer:
                filtered_out_customer_limits.append(FilteredOutLimit(limit=limit, reason="transaction has no customer"))
            else:
                ...
                if should_check:
                    ...
                    filtered_out_customer_limits.append(FilteredOutLimit(limit=limit, reason=reason))
        elif isinstance(
            limit, limit_models.MerchantLimit
        ):
            ...
            if should_check:
                ...
                filtered_out_merchant_limits.append((limit, reason or "unknown"))

    if filtered_out_customer_limits:
        logger.info(
            ...
            extra={
                ...
                "filtered_limits": [
                    {"limit_id": lim.id, "limit_type": lim.limit_type, "reason": reason}
                    for lim, reason in filtered_out_customer_limits
                ],
            },
        )

    if filtered_out_merchant_limits:
        logger.info(
            ...
            extra={
                ...
                "filtered_limits": [
                    ...
                    for lim, reason in filtered_out_merchant_limits
                ],
            },
        )
    ...
    logger.info(
        ...
        extra={
            ...
            "customer_limits_count": sum(
                1 for lim in limits_to_check if isinstance(lim, limit_models.CustomerLimit)
            ),
            "merchant_limits_count": sum(
                1 for lim in limits_to_check if isinstance(lim, limit_models.MerchantLimit)
            ),
        },
    )
    ...
    ) = _resolve_customer_and_merchant_limit_conflicts(limits_to_check)

    if customer_merchant_filtered:
        logger.info(
    ...
    logger.info(
        ...
        extra={
            "transaction_id": trx.id,
            "limit_ids": [limit.id for limit in limits_with_resolved_all_type_of_conflicts],
        },
    )
...
def _resolve_risk_and_global_risk_limit_for_merchant_conflicts(
    ...
    if removed_limits_with_reasons:
        limits_to_remove = [lim for lim, _ in removed_limits_with_reasons]
