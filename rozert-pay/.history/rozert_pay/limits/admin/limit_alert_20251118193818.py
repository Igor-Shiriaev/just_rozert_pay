...
class LimitAlertAdmin(admin.ModelAdmin):
    ...
    def limit_type_colored(self, obj: LimitAlert) -> str:
        ...
        limit_category = (
            "Risk"
            if obj.customer_limit and obj.customer_limit.category == LimitCategory.RISK
            else "Business"
        )
        color = CRITICAL_LIMIT_COLOR if obj.is_critical else REGULAR_LIMIT_COLOR
        return format_html("<span style='color: {};'>{} {} Limit</span>", color, limit_type, obj.customer_limit.category)
    ...
    def links(self, obj: LimitAlert) -> str:
        ...
        if obj.customer_limit:
            if obj.customer_limit.category == LimitCategory.RISK:
                ...
                data.append({"link": link, "name": _("Risk Customer Limit")})
            else:
                link = reverse(
                    "admin:limits_businesscustomerlimit_change",
                    args=[obj.customer_limit.pk],
                )
                data.append({"link": link, "name": _("Business Customer Limit")})
        if obj.merchant_limit:
            if obj.merchant_limit.category == LimitCategory.RISK:
                link = reverse(
                    "admin:limits_riskmerchantlimit_change",
                    args=[obj.merchant_limit.pk],
                )
                data.append({"link": link, "name": _("Risk Merchant Limit")})
            else:
                link = reverse(
                    "admin:limits_businessmerchantlimit_change",
                    args=[obj.merchant_limit.pk],
                )
                data.append({"link": link, "name": _("Business Merchant Limit")})
