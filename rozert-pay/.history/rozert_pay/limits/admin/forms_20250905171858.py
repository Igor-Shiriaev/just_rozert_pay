...
class CustomerLimitForm(ModelForm):
    ...
    def save(self, commit: bool = True) -> CustomerLimit:
        ...
        if is_update:
            ...
            for field in ['active', 'description', 'period', 'max_successful_operations', 
                         'max_failed_operations', 'min_operation_amount', 'max_operation_amount', 
                         'total_successful_amount', 'decline_on_exceed', 'is_critical']:
...
class MerchantLimitForm(ModelForm):
    ...
    def save(self, commit: bool = True) -> MerchantLimit:
        ...
        if is_update:
            ...
            # Check for changes in key fields
            for field in ['active', 'description', 'scope', 'limit_type', 'period', 
                         ...
                         'max_ratio', 'burst_minutes', 'decline_on_exceed', 'is_critical']:
                original_value = getattr(original, field)
                new_value = getattr(instance, field)
                if original_value != new_value:
                    changes.append(f"{field}: {original_value} -> {new_value}")
            ...
            if changes:
                scope_info = f"Merchant: {instance.merchant_id}" if instance.merchant_id else f"Wallet: {instance.wallet_id}"
                logger.info(
