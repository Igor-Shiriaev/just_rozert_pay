...
class BaseLimitAdmin(admin.ModelAdmin):
    ...
    actions = [
        ...
        "enable_risk_limits_globally",
        "disable_risk_limits_globally",
    ]
    ...
    @admin.display(description=_("Enable Risk limits globally"))
    def enable_risk_limits_globally(
        self, request: HttpRequest, queryset: QuerySet[MerchantLimit | CustomerLimit]
    ) -> None:
        enable_risk_limits()
        status = "enabled" if is_risk_limits_enabled() else "disabled"
        self.message_user(request, _(f"Risk limits globally {status}."))

    @admin.display(description=_("Disable Risk limits globally"))
    def disable_risk_limits_globally(
        self, request: HttpRequest, queryset: QuerySet[MerchantLimit | CustomerLimit]
    ) -> None:
        disable_risk_limits()
        status = "enabled" if is_risk_limits_enabled() else "disabled"
