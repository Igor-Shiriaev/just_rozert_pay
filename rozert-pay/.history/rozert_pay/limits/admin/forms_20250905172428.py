...
class CustomerLimitForm(ModelForm):
    ...
    def save(self, commit: bool = True) -> CustomerLimit:
        ...
        if is_update:
            ...
            if changes:
                # Log to event_logs with user info
                if self.user:
            ...
            if self.user:
                event_logs.create_event_log(
                    event_type=const.EventType.UPDATE_LIMIT,
                    description=f"CustomerLimit created by {self.user.username}",
                    extra={
                        "customer_id": instance.customer_id,
        ...
        if is_update:
            ...
            if changes:
                ...
                if self.user:
                    event_logs.create_event_log(
                        event_type=const.EventType.UPDATE_LIMIT,
                        description=f"MerchantLimit updated by {self.user.username}",
                        extra={
                            "changes": changes,
            ...
            if self.user:
                event_logs.create_event_log(
                    event_type=const.EventType.UPDATE_LIMIT,
                    description=f"MerchantLimit created by {self.user.username}",
                    extra={
                        "scope_info": scope_info,
