...
class RiskControlActionsMixin:
    ...
    def enable_risk_control(
        self: type[admin.ModelAdmin],
        request: HttpRequest,
        queryset: QuerySet[CustomerLimit],
    ) -> None:
        update_feature_flag_status(
            name=FeatureFlagName.RISK_CONTROL_ENABLED, status=True
        )
        self.message_user(request, _("Risk control enabled successfully."))

    @admin.display(description=_("Disable Risk control"))
    def disable_risk_control(
        self: type[admin.ModelAdmin],
        request: HttpRequest,
        queryset: QuerySet[CustomerLimit],
    ) -> None:
        update_feature_flag_status(
            name=FeatureFlagName.RISK_CONTROL_ENABLED, status=False
        )
