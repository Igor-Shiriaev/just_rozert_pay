...
class CustomerLimit(BaseDjangoModel):
    ...
    def clean(self) -> None:
        ...
        # Check for duplicate limits based on customer and period
        duplicate_query = CustomerLimit.objects.filter(
            customer=self.customer, period=self.period
        )

        # If this is an update (has pk), exclude current instance
        if self.pk:
            duplicate_query = duplicate_query.exclude(pk=self.pk)

        if duplicate_query.exists():
            raise ValidationError(
                "Лимит для данного клиента с таким периодом уже существует"
            )
