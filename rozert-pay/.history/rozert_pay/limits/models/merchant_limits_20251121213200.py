...
class MerchantLimit(BaseDjangoModel):
    ...
    def clean(self) -> None:
        ...
        if self.category == LimitCategory.GLOBAL_RISK and self.scope != MerchantLimitScope.WALLET:
            errors.setdefault("scope", []).append(
                "Scope must be 'wallet' for global risk limit"
            )
        if self.category == LimitCategory.GLOBAL_RISK and self.scope == MerchantLimitScope.MERCHANT:
