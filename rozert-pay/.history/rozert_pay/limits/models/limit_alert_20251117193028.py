...
class LimitAlert(BaseDjangoModel):
    ...
    def get_limit_admin_url(self) -> str:
        ...
        if self.merchant_limit:
            if self.merchant_limit.category == LimitCategory.RISK:
                return reverse(
                    "admin:limits_riskmerchantlimit_change", args=[self.merchant_limit.pk]
                )
            else:
                return reverse(
                    "admin:limits_businessmerchantlimit_change", args=[self.merchant_limit.pk]
                )
        return "#"
